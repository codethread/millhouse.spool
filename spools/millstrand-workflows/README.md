...
Before importing, parse the consumer `.clj-kondo/config.edn` once and inventory each `:config-paths` entry both as its original value and as its resolved target. Preserve every existing local config-path entry byte for byte. After the one import, manually activate every retained dependency producer export exactly once with a repository-relative path resolved from that config, normally `imports/<group>/<artifact>`. If an existing portable entry already resolves to the retained import, validate it as the one activation rather than appending a duplicate. Persist the relative entry, never its canonical or absolute target. Canonicalize the resolved target only for provenance, ownership, ambiguity, and exact-once identity comparison. An absolute or outside-repository activation fails loudly, as do missing, duplicate, ambiguous, or owned targets. The retained dependency inventory and recorded provenance are the only eligible activation sources. Do not point activation at an owned producer root or source path, discover a replacement path, or edit this by script. Owned exports must be absent from both the copied imports and `:config-paths`.

The import records owned roots, retained dependency roots, derivations, the verified Kondo command, and the final canonical-filtered classpath before one `KONDO_CMD --lint RESOLVED_CLASSPATH --dependencies --parallel --copy-configs --skip-lint` invocation. Immediately before the command, it asserts using only canonical paths that no owned producer classpath or export path, or descendant, remains in `RESOLVED_CLASSPATH`, and that every canonical owned export path is absent. Do not copy and clean up self-imports afterward. Unresolved or ambiguously owned roots, failed canonicalization, unreconcilable canonical ownership, a missing base export, an invalid base `deps.edn`, or an empty filtered installed-spool contribution fail loudly with the applicable invariant. Validation reuses the same ownership table, checks one provenance source per retained mapping, exact-once config-path activation, preservation of local paths, duplicate and overlap decisions, repository-relative self-imports, LSP ownership, and tracked cache files; legitimate remote producer imports remain valid. Quality checks are discovered from the consumer repository and handed over with the before/during/after self-import result.

When refresh reports the supported pending next-generation shape, bootstrap accepts only a top-level `:status :partial` result with a nonempty `:modules` outcome map. Every top-level `:modules` map key must equal its outcome `:module/key`; reject any map-key identity mismatch. Every module is exactly unchanged with no error/refusal/root outcome, a direct `:status :refused`/`:reason :hard-conflict` carrying the exact `:root/outcome`, or a `:status :failed`/`:status :skipped` `:reason :missing-dependency` wrapper whose `:dependency` equals the nested `:dependency/outcome` `:module/key` at every hop. Wrapper chains are finite and acyclic and terminate in an exact direct refused hard-conflict outcome. Every direct refused hard-conflict terminal, whether top-level or reached through a missing-dependency chain, must carry `:root-lib` equal to exactly one `:lib` in the declared nonempty changed-root set; reject any unrelated or mismatched terminal root-lib. Applied outcomes, other statuses/reasons, missing or mismatched dependency outcomes, cycles, unrelated terminals, and other refusals/errors fail loudly. Every direct conflict has exactly `:changed-roots` and `:namespace-residuals`; its nonempty changed-root set is the declared prepared set, and the pending-generation `:diff` equals that complete classification. Residuals map one to one to those entries, have nonempty providers, and reconcile `:root-lib`, namespaces, old/new roots, and distinct paths. The agent records current and prepared generations, verifies each prepared path, declared coordinate, cache provenance, readable `deps.edn`, and required export, then uses the prepared roots for Kondo, LSP, and tools.deps. It never falls back to the active old `sync.root` for a changed family; unchanged roots keep their active `sync.root`. With no coordinate change, accept only the recorded full `(runtime/refresh! (current/runtime))` result with top-level refresh `:status :unchanged`, every module in `:modules` with `:status :unchanged`, runtime `:pending-generation nil`, and synced-root evidence under spool status family/root `:roots` projections. Applied, partial, refused, error, other module statuses, non-nil pending generation, and malformed or contradictory evidence fail loudly. Weaver proof stays on the current generation and does not claim adoption.

For the no-coordinate proof, the pending module grammar is exact: every `:modules` map key equals its outcome `:module/key`, and every unchanged outcome has `:status :unchanged` with no `:error`, `:reason`, refusal, `:root/outcome`, `:dependency`, or `:dependency/outcome`. The selected activation and relevant producer metadata define one exact intended family/root set. Spool status must cover exactly that family/root set; every root outcome is `:status :synced` with a `:sync` map whose nonempty `:root` (`:sync.root`) matches the recorded active-root evidence. Failed, conflicted, source-reload, partial, missing, extra, mismatched, or invalid-root evidence fails loudly.
...
When alignment changes an active coordinate, the producer-proof step runs `(runtime/refresh! (current/runtime))` explicitly and records the full result. A fully applied result continues normally. The only supported pending result is the exact declared-coordinate partial shape described above; the agent records current and prepared generations and continues tooling without restart or an adoption claim. When no coordinate changed, it runs the same full refresh and records that result together with read-only `(runtime/status (current/runtime))` and `strand --workspace <workspace> spool status`. It accepts only top-level refresh `:status :unchanged`, every module in `:modules` having `:status :unchanged`, runtime `:pending-generation nil`, and synced-root evidence under spool status family/root `:roots` projections. Applied, partial, refused, error, other module statuses, non-nil pending generation, and malformed or contradictory results fail loudly.

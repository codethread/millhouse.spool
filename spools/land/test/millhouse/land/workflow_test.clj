(ns millhouse.land.workflow-test
...
(defn- register-land-routes!
  []
  (workflow/register-workflow! :land-abort 'millhouse.land/land-abort)
  (workflow/register-workflow! :land-merge 'millhouse.land/land-merge))
...
(defn- start-land!
  ...
  (workflow/start! run-id
                   @(requiring-resolve 'millhouse.land/land)
                   params))
...
(defn- reach-review-resolution!
  [rt run-id card]
  (is (= "millhouse.land.card-actions/rework-card!"
  ...
  (with-runtime
    (fn [rt _]
      ...
      (let [{:keys [root card params]} (card-fixture rt)
        ...
        (try
          (workflow/start! run-id
                           @(requiring-resolve 'millhouse.land/review)
                           params)
          (let [resolution (reach-review-resolution! rt run-id card)]
            (is (= "resolve-review" (:checkpoint resolution)))

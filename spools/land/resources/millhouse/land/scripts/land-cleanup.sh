...
# Repositories with processes or other owned resources may opt into cleanup
# through one conventional, tracked executable. The generic landing spool does
# not guess at repository-specific behavior.
cleanup_hook=.millstrand/land-cleanup.sh
if [ "$worktree_exists" = true ] \
    && git -C "$worktree" cat-file -e "$expected_head:$cleanup_hook" >/dev/null 2>&1; then
  [ -f "$worktree/$cleanup_hook" ] \
    || die "tracked cleanup hook is unavailable: $cleanup_hook"
  [ -x "$worktree/$cleanup_hook" ] \
    || die "tracked cleanup hook is not executable: $cleanup_hook"
  (cd "$worktree" \
    && LAND_EXPECTED_BRANCH=$branch LAND_EXPECTED_HEAD=$expected_head \
      "./$cleanup_hook") \
    || die "repository cleanup hook failed: $cleanup_hook"
  hook_head=$(git -C "$worktree" rev-parse HEAD) \
    || die "cannot re-read feature worktree HEAD after cleanup hook"
  [ "$hook_head" = "$expected_head" ] \
    || die "cleanup hook changed feature HEAD: expected $expected_head, found $hook_head"
  hook_status=$(git -C "$worktree" status --porcelain=v1 --untracked-files=all) \
    || die "cannot inspect feature worktree after cleanup hook"
  [ -z "$hook_status" ] \
    || die "cleanup hook left feature worktree dirty: $hook_status"
fi

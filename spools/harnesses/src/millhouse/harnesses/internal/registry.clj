(ns millhouse.harnesses.internal.registry
  "Registry helpers for concrete harnesses and aliases.

  These are named steps for `millhouse.harnesses`. Callers still go through
        ...
        (map (fn [[k v]]
               (let [k (if (keyword? k) k (keyword (str k)))]
                 ...
                 (when (and (= appended-system-prompts-attribute k)
                            (not (s/valid? :millhouse.harnesses/appended-system-prompts v)))

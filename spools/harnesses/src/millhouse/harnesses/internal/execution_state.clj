(ns millhouse.harnesses.internal.execution-state
...
(defn state-holder
  ...
  [rt]
  (runtime/spool-state rt :millhouse.harnesses.execution/state
...
(defn- deadline-hook! [payload]
  ((requiring-resolve
    'millhouse.harnesses.execution/guidance-deadline-hook!) payload))
         ...
         (when (compare-and-set! inspection-scheduled? false true)
           (.schedule
            ...
            #(do
               ...
               ((requiring-resolve
                 'millhouse.harnesses.execution/inspect-owned!) rt opened))

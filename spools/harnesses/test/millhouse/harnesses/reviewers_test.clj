(ns millhouse.harnesses.reviewers-test
...
(defn- world-deps []
  (let [harnesses-root (test-alpha/spool-checkout-root
    ...
    {:deps
     {'millhouse/harnesses {:local/root (.getCanonicalPath harnesses-root)}
      'millhouse/identity {:local/root (.getCanonicalPath identity-root)}}}))
...
(def ^:private init-source
  "(require '[millstrand.api.current.alpha :as current]
   ...
   (runtime/module! rt :harnesses
     {:ns 'millhouse.harnesses.spool :after [:identity] :required? true})
...
(deftest authoring-is-inert-and-owner-complete-per-runtime
  ...
  (with-review-world
    (fn [ctx]
      ...
      (is (= [] (test-alpha/repl!
                 ctx
                 '(do (require '[millhouse.harnesses.reviewers :as reviewers])
...
(defn- consumer-deps [repository]
  {:paths ["src"]
   :deps {'millhouse/harnesses {:local/root (.getCanonicalPath repository)}
  ...
  (let [root (.toFile (java.nio.file.Files/createTempDirectory
    ...
    (try
      (let [repository (.getCanonicalFile (io/file "."))
            _ (is (io/resource
                   "clj-kondo.exports/millhouse/harnesses/config.edn"))]
        ...
        (let [import-result (run-consumer-command!
              ...
              imported-config (io/file
                               root
                               ".clj-kondo/imports/millhouse/harnesses/config.edn")]

...
timeouts leave the gate active and ready with `gate/error`. An interrupted
in-JVM callback is different: its matching `code/running` claim is cleared
without an error stamp, leaving the gate retryable. Later scans skip a gate
while that key is present. A blank error is still present data and does not
re-arm the gate.

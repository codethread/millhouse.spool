(ns millhouse.spools.executors.code-test
  ...
  (with-code
    (fn [rt]
      (workflow/start! "nil"
                       (single-gate
                        "nil"
                        (request "millhouse.spools.executors.code-test/nil-value" {}))
                       {})
      (test-alpha/await-quiescent! rt {:timeout-ms (test-support/await-budget-ms)})
  ...
  (with-code
    (fn [rt]
      (doseq [[run-id fn-name expected]
              [["throw" "millhouse.spools.executors.code-test/throw-value" "code test exploded"]
               ["json" "millhouse.spools.executors.code-test/non-json-value" "not JSON-safe"]]]
      ...
      (doseq [[index [gate-attrs expected]]
              (map-indexed
               ...
               [[{"code/params" {}} "code/fn"]
                ...
                [(request "millhouse.spools.executors.code-test/missing" {}) "did not resolve"]
                [{"code/fn" "millhouse.spools.executors.code-test/return-value"} "code/params"]
                [(request "millhouse.spools.executors.code-test/return-value" []) "code/params"]
                [(request "millhouse.spools.executors.code-test/return-value" {} 0)
                 "code/timeout-secs"]])]
  ...
  (with-code
    (fn [rt]
      (workflow/start! "late"
                       (gated-gate
                        "late"
                        (request "millhouse.spools.executors.code-test/late-value" {}))
                       {})
      ...
      (let [run-id "saturation"
            gates (mapv (fn [index]
                          (workflow/gate
                           ...
                           :attributes
                           (assoc (request
                                   "millhouse.spools.executors.code-test/wait-for-release"
                                   {:value index})
                                  "test/run-id" run-id)))
                        (range 9))]
        (try
    ...
    (fn [rt]
      ...
      (try
        (workflow/start!
         ...
         (single-gate
          "stubborn"
          (request "millhouse.spools.executors.code-test/ignore-interrupt-until-release"
                   {:value "late"}
                   1))
         {})
        (test-alpha/await-quiescent! rt {:timeout-ms (test-support/await-budget-ms)})
    ...
    (fn [rt]
      ...
      (let [marker (temp-file)]
        (workflow/start!
         ...
         (single-gate
          "poll"
          (request "millhouse.spools.executors.code-test/poll-short-subprocesses"

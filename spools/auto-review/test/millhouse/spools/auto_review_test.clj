(ns millhouse.spools.auto-review-test
  ...
  (:require [clojure.edn :as edn]
            ...
            [millhouse.spools.auto-review.glab :as glab]
...
(def ^:private fixture
  "(ns auto-review.fixture
     (:require [ct.spools.harnesses :as harnesses]
               ...
               [ct.spools.harnesses.executors.agent :as agent]
               [millhouse.spools.auto-review :as review]
               [millhouse.spools.auto-review.workflow :as reviews]
               [millhouse.spools.workflow :as workflow]
...
(defn- root []
  (-> (t/spool-checkout-root "millhouse/spools/auto_review.clj") .getParentFile .getParentFile))
    ...
    (fn [rt config]
      (let [ordinary (revision "2")
        ...
        (testing "runtime state loss cannot forget durable dedup or capacity"
          (swap! (:spool-state rt) dissoc :millhouse.spools.auto-review/admission)
       ...
       (fn [rt config]
         (let [revision (assoc revision :ci {:status "passed" :head (:head revision)})
           ...
           (harnesses/register-harness! rt :fake
                                        {:modes #{:headless} :prepare 'ct.spools.harnesses/create!
  ...
  (with-world
    (fn [rt _]
      (cron/register! rt {:id :review-fixture :interval-ms 3600000
                          :handler 'millhouse.spools.auto-review-test/cron-poll})
      (try
        (doseq [expected [1 0]]
  ...
  (let [dir (support/temp-dir "auto-review-consumer-")]
    (try
      (spit (io/file dir "deps.edn")
            (pr-str {:deps {'millhouse.spools/auto-review
                            {:local/root (.getCanonicalPath (io/file (root) "spools/auto-review"))}}}))
      (let [result (sh/sh "clojure" "-Srepro" "-M" "-e"
                          "(require 'millhouse.spools.auto-review.glab 'millhouse.spools.auto-review.workspace 'millhouse.spools.auto-review.workflow) (println :resolved)"
                          :dir (.getPath dir))]

(ns millhouse.cron.runtime-test
  "Tests for the millhouse.cron recurrence engine against a real weaver
...
(defn- cron-root []
  (test-alpha/spool-checkout-root "millhouse/cron.clj"))
...
(deftest cron-exported-kondo-contract-is-published-by-cron-root
  (testing "Cron publishes resources from its owning root"
    ...
    (let [config-file (io/file
                       (cron-root) "resources/clj-kondo.exports/millhouse/cron/config.edn")
          config-data (edn/read-string (slurp config-file))]
      (is (.isFile config-file))
      (is (= 'clojure.core/def
             (get-in config-data [:lint-as 'millhouse.cron/defjob])))
      (is (= 'clojure.core/def
             (get-in config-data [:lint-as 'millhouse.cron/defjob!])))
      (is (= 'hooks.millstrand/use-vars
             (get-in config-data
                     [:hooks :analyze-call 'millhouse.cron/use-job!]))))))
    ...
    (fn [rt _config-dir]
      ...
      (let [handle (registry/registry)]
        (registry/declare-kind! handle {:id cron/job-kind
                                        :entry-spec :millhouse.cron/job
                                        :binding-moment :cron/fire})
        (swap! (:spool-state rt) assoc :millhouse.cron/job-kinds handle))
      (f rt))))
  ...
  (with-cron
    (fn [rt]
      ...
      (let [status (cron/register! rt {:id :slow
        ...
        (is (= :slow (:id status)))
        (is (= 'millhouse.cron.runtime-test/fire-ok (:handler status)))
        ...
        (let [wake (cron-wake rt "cron/slow")]
          ...
          (is (= 'millhouse.cron/fire-wake (:handler wake)))
    ...
    (fn [rt]
      ...
      (let [first-wake-at (:wake_at (cron-wake rt "cron/steady"))]
        ...
        (cron/register! rt {:id :steady
                            :interval-ms 2000
                            :jitter-ms 0
                            :handler 'millhouse.cron.runtime-test/fire-other})
...
(deftest owner-reconciliation-names-the-failing-job
  (with-cron
    (fn [rt]
      (let [job {:id :broken
                 :interval-ms 1000
                 :handler 'millhouse.cron.runtime-test/missing-handler}
  ...
  (with-cron
    (fn [rt]
      (cron/register! rt {:id :boom
                          :interval-ms 1000
                          :jitter-ms 0
                          :handler 'millhouse.cron.runtime-test/fire-throw})
      (release-fire! rt)
  ...
  (with-cron
    (fn [rt]
      ...
      (cron/register! rt {:id :blocked
                          :interval-ms 1000
                          :jitter-ms 0
                          :handler 'millhouse.cron.runtime-test/blocking-run})
      (test-alpha/advance! rt (Duration/ofSeconds 2))
      (test-alpha/await-quiescent! rt {:timeout-ms (test-support/await-budget-ms)})
...
(deftest job-authoring-validates-closed-options-and-job-shape
  (is (= {:kind :reconcile
          ...
          :read-desired 'millhouse.cron/desired-jobs
          :read-actual 'millhouse.cron/actual-jobs
          :apply 'millhouse.cron/apply-jobs!
          :on-removed 'millhouse.cron/remove-jobs!}
         (deref (ns-resolve 'millhouse.cron 'scheduled-jobs))))
  (is (= {:id :sample :interval-ms 1000 :handler 'millhouse.cron.runtime-test/fire-ok}
         (cron/job-declaration
          :sample {} {:interval-ms 1000 :handler 'millhouse.cron.runtime-test/fire-ok})))
  (is (thrown-with-msg? clojure.lang.ExceptionInfo #"Invalid Cron job options"
                        (cron/job-declaration
                         :sample {:unknown true}
                         {:interval-ms 1000 :handler 'millhouse.cron.runtime-test/fire-ok}))))

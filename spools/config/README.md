# Millhouse configuration

An opt-in shared bootstrap, agent/reviewer catalog, help, and Devflow election.
This is not a Millstrand core default. Install `millhouse/config` from the
...
The optional second argument adds explicit `:after` edges for consumer modules that must reconcile before the executor's initial scan of restored ready gates. `(config/register-executor! runtime)` is sufficient when there are no consumer modules to name. The stable executor module id is `:millhouse/agent-executor`; the bootstrap owns that module, so consumers must not register its namespace separately.
...
Opt-in [automatic card pickup](../../docs/processes/auto-run.md) is also available through the config root's `millhouse/auto-run` dependency. Repositories supply delivery workflows, worker defaults, worktree preparation, and a concurrency limit; the dispatcher assigns labelled, ready features once without a coordinator agent. It is not activated by the shared bootstrap.

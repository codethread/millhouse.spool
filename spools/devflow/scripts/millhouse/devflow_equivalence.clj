(ns millhouse.devflow-equivalence
...
(defn- activate! [rt]
  (doseq [namespace '[millhouse.workflow
                      millhouse.devflow
                      millhouse.kanban
...
(s/def ::divergent-params (s/keys :req-un [:millhouse.devflow.internal.definition/feature]))
...
(defn- divergence-regression! [strand]
  ...
  (let [divergent (execute-target!
                   ...
                   (fn [_]
                     ...
                     :author-card-strands
                     'millhouse.devflow-equivalence/divergent-author-card-strands)))]
    (try
      (assert-equivalent! strand divergent)
      (throw (ex-info "card-authoring divergence regression did not fire" {}))
      (catch clojure.lang.ExceptionInfo error

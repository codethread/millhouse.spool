(ns millhouse.devflow-kanban-adapter
  ...
  Requires the `millhouse/devflow` and `millhouse/kanban` roots; see
...
(s/def ::author-cards-params
  (s/and (s/keys :req-un [::feature]) :millhouse.devflow.internal.definition/landed-input))
(s/def ::runtime some?)
...
(workflow/defworkflow! decompose-kanban
  ...
  {:entrypoints #{:continue :call}
   :param-spec :millhouse.devflow.internal.definition/decompose-params
   :defaults {}}
  ...
  (let [params (if (map? params)
    ...
    (current/with-runtime runtime
      (workflow/register-workflow! :decompose
                                   'millhouse.devflow-kanban-adapter/decompose-kanban))

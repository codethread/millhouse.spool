(ns millhouse.auto-run-test
  ...
  (:require [clojure.edn :as edn]
            ...
            [millhouse.auto-run-worktree :as worktree]
...
(deftest disable-and-stale-wakes-cannot-admit-work
  (with-world
    (fn [rt config]
      (let [card (card! rt {})
            wake (some #(when (= "millhouse/auto-run" (:key %)) %) (scheduler/pending rt))]
        ...
        (auto-run/wake! {:runtime rt :payload (:payload wake)})
        (is (nil? (show rt card :auto-run/status)))
        (let [fresh (some #(when (= "millhouse/auto-run" (:key %)) %) (scheduler/pending rt))]
  ...
  (with-world
    (fn [rt config]
      (let [card (card! rt {})
        ...
        (is (= before (snapshot)))
        (is (= "millhouse.auto-run.explain/v1" (:schema-version unsupported)))

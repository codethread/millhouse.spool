...
(defn- activate-kanban!
  ...
  [rt]
  (runtime/module! rt :identity {:ns 'millhouse.identity})
  (let [result (runtime/module! rt :kanban {:ns 'millhouse.kanban})
...
(defn- kanban-surface [rt]
  {:ops (->> (weaver/ops rt)
             (filter #(= 'millhouse.kanban (:provenance %)))
   ...
   (fn [ctx]
     (let [rt (:runtime ctx)
           ...
           image-result (runtime/module! rt :kanban
                                         {:ns 'millhouse.kanban :load :image})
...
(deftest omitting-kanban-retracts-static-entries-and-closes-its-resource
  (let [init-source
        (str "(require '[millstrand.api.current.alpha :as current]\n"
             ...
             "(runtime/module! runtime :identity {:ns 'millhouse.identity})\n"
             "(runtime/module! runtime :kanban {:ns 'millhouse.kanban})\n")]
    (t/run-with-weaver-world
     {:storage :sqlite-memory
      ...
      (let [entry (weaver/resolve-op rt 'kanban)
        ...
        (testing "cross-verb narrative is op metadata for the built-in meta-verbs"
          ...
          (doseq [guidance [about prime]]
            (is (str/includes? guidance "the user needs to act"))
            (is (str/includes? guidance "Ready may be dependency-blocked"))
            (is (str/includes? guidance "an agent is actually"))
            (is (str/includes? guidance "handoff"))
            (is (str/includes? guidance "await-only watcher"))
            (is (str/includes? guidance "failure evidence"))
...
(defn -main
  ...
  [& _args]
  (let [summary (clojure.test/run-tests 'millhouse.kanban-test)]

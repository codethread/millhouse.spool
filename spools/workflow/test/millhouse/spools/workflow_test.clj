(ns millhouse.spools.workflow-test
...
(deftest workflow-spool-explains-public-input-shapes
  (let [contract (workflow/explain)]
    ...
    (is (= 'millhouse.spools.workflow/checkpoint (get-in contract [:builders 'checkpoint])))
    (is (re-find #"millhouse.spools.workflow/workflow" (get-in contract [:contract :spec])))
    ...
    (is (= 'millhouse.spools.workflow/defworkflow
           (get-in contract [:builders 'defworkflow])))
    (is (re-find #"millhouse.spools.workflow/definition"
      ...
      (let [toastie (workflow/workflow
                     ...
                     (workflow/checkpoint :choose-finish "Choose toastie finish"
                                          ...
                                          :choices [{:key :serve
                                                     ...
                                                     :next 'millhouse.spools.workflow-test/toastie-serve-workflow}
                                                    {:key :remake
                                                     :label "Remake"
  ...
  (workflow/workflow
   ...
   (workflow/checkpoint :ci-verdict "Judge CI result"
                        ...
                        :choices [{:key :green
                                   ...
                                   :next 'millhouse.spools.workflow-test/pr-review-round}
                                  {:key :red
                                   :label "CI red"
                                   :description "Checks failed; run the fix-CI loop."
                                   :next 'millhouse.spools.workflow-test/pr-fix-ci}])))
  ...
  (workflow/workflow
   ...
   (workflow/checkpoint :review-verdict "Judge review outcome"
                        ...
                        :choices [{:key :approved
                                   ...
                                   :next 'millhouse.spools.workflow-test/pr-merge}
                                  {:key :changes-requested
                                   :label "Changes requested"
                                   :description "Address comments, push, and re-run CI."
                                   :next 'millhouse.spools.workflow-test/pr-fix-and-push}])))
    ...
    (fn [rt _]
      (let [definition (workflow/workflow
        ...
        (hooks/register-hook! rt :reject-complete #{:batch/apply-before-commit}
                              'millhouse.spools.workflow-test/reject-complete-batch-hook {})
        (let [thrown (try
  ...
  (with-runtime
    (fn [rt _]
      (let [definition (workflow/workflow
        ...
        (is (= "active" (:state (weaver/show rt (:id step)))))
        (is (not (s/valid? :millhouse.spools.workflow.request/context
                           {:opaque (Object.)})))))))
    ...
    (fn [_rt _]
      (let [definition (workflow/workflow
                        ...
                        (workflow/checkpoint :route "Route somewhere"
                                             :choices [{:key :finish
                                                        :label "Finish"
                                                        :next 'millhouse.spools.workflow-test/empty-continuation-workflow}]))]
      ...
      (let [definition (workflow/workflow
                        ...
                        (workflow/checkpoint :route "Route somewhere"
                                             :choices [{:key :continue
                                                        :label "Continue"
                                                        :next 'millhouse.spools.workflow-test/routed-continuation-workflow}]))]
  ...
  (workflow/workflow
   ...
   (workflow/checkpoint :signoff "Sign off"
                        ...
                        :choices [{:key :approved :label "Approve"}
                                  {:key :revise
                                   :label "Revise"
                                   :next 'millhouse.spools.workflow-test/loopy-revision}])))
  ...
  (with-runtime
    (fn [_rt _]
      ...
      (let [root (workflow/current-root "var-start")]
        (is (= "millhouse.spools.workflow-test/loopy"
...
(deftest workflow-start-accepts-registered-keyword
  (with-runtime
    (fn [_rt _]
      (workflow/register-workflow! :loopy-test 'millhouse.spools.workflow-test/loopy)
...
(deftest workflow-describe-accepts-registered-keyword
  (with-runtime
    (fn [_rt _]
      (workflow/register-workflow! :loopy-describe 'millhouse.spools.workflow-test/loopy)
...
(workflow/defworkflow cyclic-procedure
  ...
  (workflow/workflow "Cyclic procedure"
                     ...
                     (workflow/call :again 'millhouse.spools.workflow-test/cyclic-procedure {}
                                    :depends-on [:work])))
...
(deftest workflow-compile-resolves-symbol-procedures
  (let [payload (workflow/compile
                 (workflow/workflow "Symbol procedure demo"
                                    (workflow/call :quality 'millhouse.spools.workflow-test/toastie-quality-workflow {})))]
...
(deftest workflow-choice-input-rejects-the-removed-vector-declaration
  ...
  (let [data (ex-data (try
    ...
    (is (= :millhouse.spools.workflow/choices (::s/spec (:explain data))))
...
(deftest workflow-named-next-resolves-and-fails-loudly-on-unknown-name
  (with-runtime
    (fn [rt _]
      (workflow/register-workflow! :wt-second 'millhouse.spools.workflow-test/registry-second-stage)
      (is (= 'millhouse.spools.workflow-test/registry-second-stage
...
(deftest workflow-registry-rename-repoints-in-flight-run
  (with-runtime
    (fn [_rt _]
      (workflow/register-workflow! :wt-rename 'millhouse.spools.workflow-test/registry-second-stage)
      ...
      (workflow/register-workflow! :wt-rename 'millhouse.spools.workflow-test/registry-alt-second-stage)
      (is (= ["Do alt"]
  ...
  (with-runtime
    (fn [rt _]
      ...
      (workflow/register-workflow! :wt-downstream
                                   'millhouse.spools.workflow-test/downstream-stage-workflow)
...
(deftest workflow-revise-repours-definition-skipping-condition-gated-steps
  (with-runtime
    (fn [_rt _]
      (workflow/register-workflow! :wt-downstream 'millhouse.spools.workflow-test/downstream-stage-workflow)
  ...
  (workflow/workflow
   ...
   (workflow/checkpoint :signoff "Sign off"
                        ...
                        :choices [{:key :approve
                                   :label "Approve"
                                   :description "Ship it."
                                   :next 'millhouse.spools.workflow-test/introspect-stage-b}
                                  {:key :revise
                                   :label "Revise"
...
(deftest workflow-describe-projects-choices-input-and-condition-filtering
  ...
  (let [desc (workflow/describe #'introspect-stage-a {:feature "widgets"})
    ...
    (is (= "millhouse.spools.workflow-test/introspect-stage-b"
           (:next (get choices "approve"))))
    (is (= {:revision true} (:revise (get choices "revise"))))
    (is (= {"spec" "millhouse.spools.workflow-test/revise-reason-input"
            "doc" "Why revise"}
...
(deftest executor-symbol-resolves-to-a-function-value-per-gate-evaluation
  ...
  (with-runtime
    (fn [rt _]
      (test-support/activate-spool! rt :millhouse/spools-workflow 'millhouse.spools.workflow)
      (workflow/register-executor! :exec-snap 'millhouse.spools.workflow-test/exec-detail-a)
      (let [snapshot (wf-registry/executor-for rt "exec-snap")]
        (is (= {:by :a} (snapshot {})))
        (workflow/register-executor! :exec-snap 'millhouse.spools.workflow-test/exec-detail-b)
...
(deftest executor-unresolved-stall-symbol-fails-loudly
  ...
  (with-runtime
    (fn [rt _]
      (test-support/activate-spool! rt :millhouse/spools-workflow 'millhouse.spools.workflow)
      (workflow/register-executor! :exec-gone 'millhouse.spools.workflow-test/no-such-predicate)
      (let [err (is (thrown-with-msg? clojure.lang.ExceptionInfo
        ...
        (is (= 'millhouse.spools.workflow-test/no-such-predicate (:stalled? (ex-data err)))))
  ...
  (with-runtime
    (fn [rt _]
      ...
      (workflow/register-executor!
       :decl-exec {:stalled? 'millhouse.spools.workflow-test/exec-detail-a
      ...
      (let [item (first (filter #(= "decl-exec" (:waiter %))
        ...
        (is (= "millhouse.spools.workflow-test/exec-detail-a" (:stall-predicate item)))
        (is (= "millhouse.spools.workflow-test/decl-exec-request"
  ...
  (with-runtime
    (fn [rt _]
      ...
      (is (thrown-with-msg? clojure.lang.ExceptionInfo #"Executor declaration map"
                            (workflow/register-executor!
                             :bad-decl {:stalledd? 'millhouse.spools.workflow-test/exec-detail-a}))))))
...
(deftest executor-catalog-fails-loudly-on-an-unresolvable-request-spec
  ...
  (with-runtime
    (fn [rt _]
      (test-support/activate-spool! rt :millhouse/spools-workflow 'millhouse.spools.workflow)
      (workflow/register-executor!
       :stale-decl {:stalled? 'millhouse.spools.workflow-test/exec-detail-a
      ...
      (let [handle (wf-registry/registry-handle rt)
            ...
            spools-executors (fn [entries]
                               (registry/replace-owner!
                                ...
                                {:layer :spools :entries entries :overrides #{}}))]
        (spools-definitions {:route-a 'millhouse.spools.workflow-test/registry-second-stage
                             :route-b 'millhouse.spools.workflow-test/registry-alt-second-stage})
        (spools-executors {"exec-a" 'millhouse.spools.workflow-test/exec-detail-a
                           "exec-b" 'millhouse.spools.workflow-test/exec-detail-b})
    ...
    (fn [rt _]
      ...
      (let [handle (wf-registry/registry-handle rt)]
        (registry/replace-owner! handle workflow/definition-kind :spools/pkg
                                 {:layer :spools
                                  :entries {:route-x 'millhouse.spools.workflow-test/registry-second-stage}
                                  :overrides #{}})
        (is (= 'millhouse.spools.workflow-test/registry-second-stage
               (workflow/workflow-definition :route-x)))
        ;; a direct/REPL registration shadows the lower spools layer
        (workflow/register-workflow! :route-x 'millhouse.spools.workflow-test/registry-alt-second-stage)
        (is (= 'millhouse.spools.workflow-test/registry-alt-second-stage
               (workflow/workflow-definition :route-x))
...
(deftest static-definition-start-merges-defaults-and-records-identity
  (with-runtime
    (fn [rt _]
      (test-support/activate-spool! rt :millhouse/spools-workflow 'millhouse.spools.workflow)
      (workflow/register-workflow! :wt-build 'millhouse.spools.workflow-test/static-build)
      ...
      (let [root (workflow/current-root "static-run")]
        ...
        (is (= "millhouse.spools.workflow-test/static-build"
               (get-in root [:attributes :workflow/definition]))
...
(deftest static-definition-start-requires-the-start-entrypoint
  (with-runtime
    (fn [rt _]
      (test-support/activate-spool! rt :millhouse/spools-workflow 'millhouse.spools.workflow)
      (workflow/register-workflow! :wt-review 'millhouse.spools.workflow-test/static-review)
...
(deftest unregister-workflow-removes-the-direct-registration
  (with-runtime
    (fn [rt _]
      ...
      (workflow/register-workflow! :wt-build 'millhouse.spools.workflow-test/static-build)
      (workflow/register-workflow! :wt-review 'millhouse.spools.workflow-test/static-review)
      (is (= {:wt-review 'millhouse.spools.workflow-test/static-review}
  ...
  (with-runtime
    (fn [rt _]
      ...
      (let [thrown (try (workflow/register-workflow!
                         :wt-missing 'millhouse.spools.workflow-test/no-such-definition)
                        (catch clojure.lang.ExceptionInfo e e))
            data (ex-data thrown)]
        (is (= :workflow/definition-unresolvable (:reason data)))
        (is (= :wt-missing (:name data)))
        (is (= 'millhouse.spools.workflow-test/no-such-definition (:definition data)))
        (is (= 'millhouse.spools.workflow-test (:namespace data)))
        (is (seq (:repair data))))
      (is (empty? (workflow/workflows))
...
(deftest register-workflow-rejects-a-route-to-a-missing-target
  (with-runtime
    (fn [rt _]
      (test-support/activate-spool! rt :millhouse/spools-workflow 'millhouse.spools.workflow)
      (let [thrown (try (workflow/register-workflow!
                         :wt-spike 'millhouse.spools.workflow-test/static-spike)
                        (catch clojure.lang.ExceptionInfo e e))]
...
(deftest register-workflow-rejects-non-json-defaults-and-unknown-param-specs
  (with-runtime
    (fn [rt _]
      (test-support/activate-spool! rt :millhouse/spools-workflow 'millhouse.spools.workflow)
      (let [thrown (try (workflow/register-workflow!
                         :wt-bad-defaults 'millhouse.spools.workflow-test/bad-defaults-definition)
                        (catch clojure.lang.ExceptionInfo e e))]
        (is (= :workflow/defaults-invalid (:reason (ex-data thrown))))
        (is (= [:at] (:path (ex-data thrown)))))
      (let [thrown (try (workflow/register-workflow!
                         :wt-bad-spec 'millhouse.spools.workflow-test/bad-spec-definition)
                        (catch clojure.lang.ExceptionInfo e e))]
...
(deftest a-registered-symbol-must-resolve-to-a-definition-map
  ...
  (with-runtime
    (fn [rt _]
      (test-support/activate-spool! rt :millhouse/spools-workflow 'millhouse.spools.workflow)
      (doseq [[label sym] [["a function" 'millhouse.spools.workflow-test/exploding-constructor]
                           ["a non-workflow value" 'millhouse.spools.workflow-test/malformed-constructor]]]
...
(deftest revision-resolves-the-live-registered-name
  (with-runtime
    (fn [rt _]
      (test-support/activate-spool! rt :millhouse/spools-workflow 'millhouse.spools.workflow)
      (workflow/register-workflow! :wt-revisable 'millhouse.spools.workflow-test/revisable-definition)
      ...
      (workflow/register-workflow! :wt-revisable 'millhouse.spools.workflow-test/revisable-definition-v2)
      (workflow/choose! "revise-name-run" :again)
      (is (= "Revisable v2" (:title (workflow/current-root "revise-name-run"))))
      (is (= "millhouse.spools.workflow-test/revisable-definition-v2"
  ...
  (with-runtime
    (fn [rt _]
      ...
      (let [original @#'live-var-definition]
        (try
          (workflow/register-workflow! :wt-live 'millhouse.spools.workflow-test/live-var-definition)
...
(deftest spec-first-params-merge-defaults-before-whole-map-validation
  (with-runtime
    (fn [rt _]
      (test-support/activate-spool! rt :millhouse/spools-workflow 'millhouse.spools.workflow)
      (workflow/register-workflow! :wt-spec-build 'millhouse.spools.workflow-test/spec-first-build)
...
(deftest spec-first-revision-validates-its-override-params
  (with-runtime
    (fn [rt _]
      (test-support/activate-spool! rt :millhouse/spools-workflow 'millhouse.spools.workflow)
      (workflow/register-workflow! :wt-revisable 'millhouse.spools.workflow-test/spec-first-revisable)
    ...
    (fn [rt _]
      ...
      (testing "the poured checkpoint records identity, doc, and the form graph"
        (let [detail (workflow/choice-detail "input-run" :approve)
          ...
          (is (= "millhouse.spools.workflow-test/approval-input" (get declared "spec")))
          ...
          (is (= [{"spec" "millhouse.spools.workflow-test/approval-input"
                   "relation" "root"
                   "form" (pr-str (s/form ::approval-input))}
                  {"spec" "millhouse.spools.workflow-test/approval-note"
...
(deftest definition-view-merges-authored-docs-and-example
  (with-runtime
    (fn [rt _]
      (test-support/activate-spool! rt :millhouse/spools-workflow 'millhouse.spools.workflow)
      (workflow/register-workflow! :wt-authored 'millhouse.spools.workflow-test/authored-build)
      (let [params (:params (workflow/definition-view :wt-authored))
        ...
        (testing "a definition without authored fields shows none"
          (workflow/register-workflow!
           :wt-plain 'millhouse.spools.workflow-test/spec-first-build)
...
(deftest choices-view-spec-rejects-malformed-details
  (let [choices :millhouse.spools.workflow.view.choices/choices]
...
(deftest registering-a-definition-with-an-unknown-input-spec-is-refused
  (with-runtime
    (fn [rt _]
      (test-support/activate-spool! rt :millhouse/spools-workflow 'millhouse.spools.workflow)
      (let [thrown (try (workflow/register-workflow!
                         :wt-bad-input 'millhouse.spools.workflow-test/unknown-input-definition)
                        (catch clojure.lang.ExceptionInfo e e))]
        (is (= :workflow/input-spec-missing (:reason (ex-data thrown))))
...
(deftest spec-forms-walks-nested-keys-collections-and-aliases
  (is (= [["millhouse.spools.workflow-test/node" "root"]
          ["millhouse.spools.workflow-test/children" "keyword-reference"]
          ["millhouse.spools.workflow-test/label" "keyword-reference"]]
         (form-graph ::node))
  ...
  (is (= [["millhouse.spools.workflow-test/node-alias" "root"]
          ["millhouse.spools.workflow-test/children" "keyword-reference"]
          ["millhouse.spools.workflow-test/label" "keyword-reference"]
          ["millhouse.spools.workflow-test/node" "keyword-reference"]]
...
(deftest spec-forms-is-cycle-safe-and-deterministic
  ...
  (is (= 1 (count (filter #(= "millhouse.spools.workflow-test/node" (first %))
                          (form-graph ::node))))))

(deftest spec-forms-reports-keyword-literals-without-claiming-dependency
  (is (= [["millhouse.spools.workflow-test/stage-enum" "root"]
          ["millhouse.spools.workflow-test/draft" "keyword-reference"]]
...
(deftest spec-forms-executes-no-predicate
  ...
  (is (= [["millhouse.spools.workflow-test/counting-params" "root"]
          ["millhouse.spools.workflow-test/counted" "keyword-reference"]]
...
(deftest registered-call-targets-validate-their-params
  ...
  (with-runtime
    (fn [rt _]
      (test-support/activate-spool! rt :millhouse/spools-workflow 'millhouse.spools.workflow)
      (workflow/register-workflow! :wt-callable 'millhouse.spools.workflow-test/spec-first-callable)
...
(defn- register-defer-targets! []
  (workflow/register-workflow! :wt-devflow 'millhouse.spools.workflow-test/defer-devflow)
  (workflow/register-workflow! :wt-spike 'millhouse.spools.workflow-test/defer-spike))
...
(deftest registering-a-definition-with-an-unbound-defer-is-refused
  (with-runtime
    (fn [rt _]
      (test-support/activate-spool! rt :millhouse/spools-workflow 'millhouse.spools.workflow)
      (let [thrown (try (workflow/register-workflow!
                         :wt-template 'millhouse.spools.workflow-test/card-template)
                        (catch clojure.lang.ExceptionInfo e e))]
        (is (= :workflow/defer-unbound (:reason (ex-data thrown)))))
  ...
  (with-runtime
    (fn [rt _]
      ...
      (testing "a target that is not registered at all"
        (let [thrown (try (workflow/register-workflow!
                           :wt-card 'millhouse.spools.workflow-test/tracked-card)
                          (catch clojure.lang.ExceptionInfo e e))]
          (is (= :workflow/reference-unregistered (:reason (ex-data thrown))))
          (is (= :call (:entrypoint (ex-data thrown)))
      ...
      (testing "a target that declares only :continue is not selectable"
        (workflow/register-workflow! :wt-continueonly
                                     'millhouse.spools.workflow-test/defer-continue-only)
        (let [thrown (try (workflow/register-workflow!
                           :wt-bad-card
      ...
      (testing "a target that is not a definition map at all"
        (let [thrown (try (workflow/register-workflow!
                           :wt-legacy 'millhouse.spools.workflow-test/exploding-constructor)
                          (catch clojure.lang.ExceptionInfo e e))]
          (is (= :workflow/definition-invalid (:reason (ex-data thrown))))
          (is (= 'millhouse.spools.workflow-test/exploding-constructor
...
(deftest a-call-procedure-may-declare-a-defer
  ...
  (with-runtime
    (fn [rt _]
      (test-support/activate-spool! rt :millhouse/spools-workflow 'millhouse.spools.workflow)
      (register-defer-targets!)
      (workflow/register-workflow! :wt-card 'millhouse.spools.workflow-test/tracked-card)
      (is (= :wt-caller (workflow/register-workflow!
                         :wt-caller 'millhouse.spools.workflow-test/defer-caller)))
  ...
  (with-runtime
    (fn [rt _]
      ...
      (workflow/register-workflow! :wt-two-step
                                   'millhouse.spools.workflow-test/defer-two-step-target)
          ...
          (let [filled (workflow/defer! "sandwich" :wt-two-step
            ...
            (testing "CC6: the fill record"
              (let [attrs (:attributes join)]
                ...
                (is (= "millhouse.spools.workflow-test/defer-two-step-target"
  ...
  (with-runtime
    (fn [rt _]
      ...
      (testing "a compatible repoint runs the replacement"
        (workflow/register-workflow! :wt-spike 'millhouse.spools.workflow-test/defer-devflow)
...
(deftest compile-stamps-each-defer-with-its-lexical-path
  (let [callee (workflow/bind-defers
        ...
        payload (workflow/compile outer {} {:definition 'millhouse.spools.workflow-test/outer})
        path (get-in (first (filter #(= "defer" (get-in % [:attributes "workflow/role"]))
                                    (:strands payload)))
                     [:attributes "workflow/defer-path"])]
    (is (= ["millhouse.spools.workflow-test/outer" nil] (mapv #(get % "definition") path))
    ...
    (let [forged {:id :again :title "Choose again"
      ...
      (testing "and compiling the raw map anyway overwrites the authored value"
        (let [payload (workflow/compile {:name "Forged" :steps [forged]} {}
                                        {:definition 'millhouse.spools.workflow-test/forged})
              path (get-in (second (:strands payload))
                           [:attributes "workflow/defer-path"])]
          (is (= ["millhouse.spools.workflow-test/forged"] (mapv #(get % "definition") path))
...
(deftest defer-refuses-a-direct-cycle-and-permits-siblings
  ...
  (with-runtime
    (fn [rt _]
      ...
      (workflow/register-workflow! :wt-two-step
                                   'millhouse.spools.workflow-test/defer-two-step-target)
      (workflow/register-workflow! :wt-self 'millhouse.spools.workflow-test/defer-self-target)
  ...
  (with-runtime
    (fn [rt _]
      ...
      (workflow/register-workflow! :wt-nested-inner
                                   'millhouse.spools.workflow-test/defer-nested-inner)
      (workflow/register-workflow! :wt-nested-outer
                                   'millhouse.spools.workflow-test/defer-nested-outer)
  ...
  (with-runtime
    (fn [rt _]
      ...
      (workflow/register-workflow! :wt-cycle-b
                                   'millhouse.spools.workflow-test/defer-two-step-target)
      (workflow/register-workflow! :wt-cycle-a 'millhouse.spools.workflow-test/defer-cycle-a)
      (workflow/register-workflow! :wt-cycle-b 'millhouse.spools.workflow-test/defer-cycle-b)
...
(deftest defer-materializes-a-multi-step-expansion-with-colliding-ids
  ...
  (with-runtime
    (fn [rt _]
      (test-support/activate-spool! rt :millhouse/spools-workflow 'millhouse.spools.workflow)
      (workflow/register-workflow! :wt-fanout 'millhouse.spools.workflow-test/defer-fanout-target)
...
(deftest defer-into-an-empty-target-does-not-stall-the-run
  ...
  (with-runtime
    (fn [rt _]
      (test-support/activate-spool! rt :millhouse/spools-workflow 'millhouse.spools.workflow)
      (workflow/register-workflow! :wt-empty 'millhouse.spools.workflow-test/defer-empty-target)
    ...
    (fn [rt _]
      ...
      (let [pending (workflow/ready-step "atomic")
        ...
        (hooks/register-hook! rt :reject-defer #{:batch/apply-before-commit}
                              'millhouse.spools.workflow-test/reject-defer-batch-hook {})
  ...
  (with-runtime
    (fn [rt _]
      ...
      (workflow/register-workflow! :wt-second
                                   'millhouse.spools.workflow-test/registry-second-stage)
      (let [definition (workflow/bind-defers
                        (workflow/workflow
...
(deftest the-removed-dispatch-and-transfer-surface-is-gone
  ...
  (doseq [removed '[dispatch dispatch! run-dispatch! continue! run-continue!
                    bind-handoffs]]
    (is (nil? (ns-resolve 'millhouse.spools.workflow removed))
        (str removed " must not survive as a public var")))
  (doseq [removed [:millhouse.spools.workflow/dispatch-declaration
                   :millhouse.spools.workflow/dispatch-request
                   :millhouse.spools.workflow/continue-request
                   :millhouse.spools.workflow/handoff-bindings]]
    (is (nil? (s/get-spec removed)) (str removed " must not survive as a spec")))
  (is (= #{"step" "checkpoint" "defer"}
         (set (s/form :millhouse.spools.workflow.view/role)))
...
(deftest executor-authoring-validates-closed-options
  (is (= {:stalled? 'millhouse.spools.workflow-test/exec-detail-a
          :request-spec ::request}
         (workflow/executor-declaration
          {:request-spec ::request}
          'millhouse.spools.workflow-test/exec-detail-a)))

(ns millhouse.spools.workflow-cli-test
...
(defn- register!
  ...
  (doseq [name names]
    (workflow/register-workflow!
     name (symbol "millhouse.spools.workflow-cli-test" (clojure.core/name name)))))
...
(deftest activating-the-workflow-engine-publishes-no-cli-ops
  ...
  (with-runtime
    (fn [rt _]
      ...
      (is (nil? (ns-resolve 'millhouse.spools.workflow 'spool))
  ...
  (with-runtime
    (fn [rt _]
      ...
      (let [entry (weaver/resolve-op rt 'workflow)]
        ...
        (is (nil? (ns-resolve 'millhouse.spools.workflow.cli 'spool))
  ...
  (with-runtime
    (fn [rt _]
      ...
      (is (= [{:name "build"
               ...
               :entrypoints ["start" "continue" "call"]
               :definition "millhouse.spools.workflow-cli-test/build"}
              {:name "review"
               :doc "Review a completed implementation."
               :entrypoints ["start" "call"]
               :definition "millhouse.spools.workflow-cli-test/review"}
              {:name "spike"
               :doc "Reduce uncertainty and recommend the next routine."
               :entrypoints ["start" "continue"]
               :definition "millhouse.spools.workflow-cli-test/spike"}]
             (listed))
    ...
    (fn [rt _]
      ...
      (is (= :workflow/definition-invalid
             (reason-of #(workflow/register-workflow!
                          :legacy 'millhouse.spools.workflow-cli-test/legacy-spike)))))))
...
(deftest discovery-fails-loudly-on-a-definition-whose-var-vanished
  ...
  (with-runtime
    (fn [rt _]
      (activate-cli! rt)
      (register! :build)
      (wf-registry/register-definition! rt :ghost 'millhouse.spools.workflow-cli-test/no-such-definition)
      (let [data (ex-data (try (listed) (catch clojure.lang.ExceptionInfo e e)))]
        (is (= :workflow/definition-unresolvable (:reason data)))
        (is (= :ghost (:name data)))
        (is (= 'millhouse.spools.workflow-cli-test/no-such-definition (:definition data)))
    ...
    (fn [rt _]
      ...
      (let [view (shown :spike)]
        ...
        (testing "the param contract is the live spec, its form graph, and the defaults"
          (is (= {:kind "spec"
                  :defaults {:prototype-targets ["compact queue"]}
                  :spec "millhouse.spools.workflow-cli-test/spike-params"
                  :spec-forms [{"spec" "millhouse.spools.workflow-cli-test/spike-params"
                                "relation" "root"
                                "form" (pr-str (s/form ::spike-params))}
                               {"spec" "millhouse.spools.workflow-cli-test/brief"
                                "relation" "keyword-reference"
                                "form" "clojure.core/string?"}
                               {"spec" "millhouse.spools.workflow-cli-test/scope"
                                "relation" "keyword-reference"
                                "form" "clojure.core/string?"}]}
                 (-> (:params view)
      ...
      (let [result (cli/workflow {:op/args {:subcommand ["executors"]}})
        ...
        (testing "a declared executor projects its gate-request contract"
          (let [item (by-waiter "shell")
            ...
            (is (= "millhouse.spools.executors.shell/shell-stalled?"
                   (:stall-predicate item)))
            (is (= "millhouse.spools.executors.shell/request" (:spec request)))
            (is (= ["shell/argv"]
        ...
        (testing "a bare-symbol executor lists with no contract"
          (is (= {:waiter "bare-sym"
                  :stall-predicate "millhouse.spools.workflow-cli-test/never-stalled"}
                 (by-waiter "bare-sym"))))

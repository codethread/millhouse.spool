(ns millhouse.workflow-test
  "Tests for the millhouse.workflow userland workflow engine: contract
      ...
      (let [toastie (workflow/workflow
                     ...
                     (workflow/checkpoint :choose-finish "Choose toastie finish"
                                          ...
                                          :choices [{:key :serve
                                                     ...
                                                     :next 'millhouse.workflow-test/toastie-serve-workflow}
                                                    {:key :remake
                                                     :label "Remake"
  ...
  (workflow/workflow
   ...
   (workflow/checkpoint :ci-verdict "Judge CI result"
                        ...
                        :choices [{:key :green
                                   ...
                                   :next 'millhouse.workflow-test/pr-review-round}
                                  {:key :red
                                   :label "CI red"
                                   :description "Checks failed; run the fix-CI loop."
                                   :next 'millhouse.workflow-test/pr-fix-ci}])))
  ...
  (workflow/workflow
   ...
   (workflow/checkpoint :review-verdict "Judge review outcome"
                        ...
                        :choices [{:key :approved
                                   ...
                                   :next 'millhouse.workflow-test/pr-merge}
                                  {:key :changes-requested
                                   :label "Changes requested"
                                   :description "Address comments, push, and re-run CI."
                                   :next 'millhouse.workflow-test/pr-fix-and-push}])))
    ...
    (fn [rt _]
      (let [definition (workflow/workflow
        ...
        (hooks/register-hook! rt :reject-complete #{:batch/apply-before-commit}
                              'millhouse.workflow-test/reject-complete-batch-hook {})
        (let [thrown (try
    ...
    (fn [_rt _]
      (let [definition (workflow/workflow
                        ...
                        (workflow/checkpoint :route "Route somewhere"
                                             :choices [{:key :finish
                                                        :label "Finish"
                                                        :next 'millhouse.workflow-test/empty-continuation-workflow}]))]
      ...
      (let [definition (workflow/workflow
                        ...
                        (workflow/checkpoint :route "Route somewhere"
                                             :choices [{:key :continue
                                                        :label "Continue"
                                                        :next 'millhouse.workflow-test/routed-continuation-workflow}]))]
  ...
  (workflow/workflow
   ...
   (workflow/checkpoint :signoff "Sign off"
                        ...
                        :choices [{:key :approved :label "Approve"}
                                  {:key :revise
                                   :label "Revise"
                                   :next 'millhouse.workflow-test/loopy-revision}])))
  ...
  (with-runtime
    (fn [_rt _]
      ...
      (let [root (workflow/current-root "var-start")]
        (is (= "millhouse.workflow-test/loopy"
...
(deftest workflow-start-accepts-registered-keyword
  (with-runtime
    (fn [_rt _]
      (workflow/register-workflow! :loopy-test 'millhouse.workflow-test/loopy)
...
(deftest workflow-describe-accepts-registered-keyword
  (with-runtime
    (fn [_rt _]
      (workflow/register-workflow! :loopy-describe 'millhouse.workflow-test/loopy)
...
(deftest workflow-named-next-resolves-and-fails-loudly-on-unknown-name
  (with-runtime
    (fn [rt _]
      (workflow/register-workflow! :wt-second 'millhouse.workflow-test/registry-second-stage)
      (is (= 'millhouse.workflow-test/registry-second-stage
...
(deftest workflow-registry-rename-repoints-in-flight-run
  (with-runtime
    (fn [_rt _]
      (workflow/register-workflow! :wt-rename 'millhouse.workflow-test/registry-second-stage)
      ...
      (workflow/register-workflow! :wt-rename 'millhouse.workflow-test/registry-alt-second-stage)
      (is (= ["Do alt"]
  ...
  (with-runtime
    (fn [rt _]
      ...
      (workflow/register-workflow! :wt-downstream
                                   'millhouse.workflow-test/downstream-stage-workflow)
...
(deftest workflow-revise-repours-definition-skipping-condition-gated-steps
  (with-runtime
    (fn [_rt _]
      (workflow/register-workflow! :wt-downstream 'millhouse.workflow-test/downstream-stage-workflow)
  ...
  (workflow/workflow
   ...
   (workflow/checkpoint :signoff "Sign off"
                        ...
                        :choices [{:key :approve
                                   :label "Approve"
                                   :description "Ship it."
                                   :next 'millhouse.workflow-test/introspect-stage-b}
                                  {:key :revise
                                   :label "Revise"
...
(deftest workflow-describe-projects-choices-input-and-condition-filtering
  ...
  (let [desc (workflow/describe #'introspect-stage-a {:feature "widgets"})
    ...
    (is (= "millhouse.workflow-test/introspect-stage-b"
           (:next (get choices "approve"))))
    (is (= {:revision true} (:revise (get choices "revise"))))
    (is (= {"spec" "millhouse.workflow-test/revise-reason-input"
            "doc" "Why revise"}
...
(deftest executor-symbol-resolves-to-a-function-value-per-gate-evaluation
  ...
  (with-runtime
    (fn [rt _]
      (test-support/activate-spool! rt :millhouse/workflow 'millhouse.workflow)
      (workflow/register-executor! :exec-snap 'millhouse.workflow-test/exec-detail-a)
      (let [snapshot (wf-registry/executor-for rt "exec-snap")]
        (is (= {:by :a} (snapshot {})))
        (workflow/register-executor! :exec-snap 'millhouse.workflow-test/exec-detail-b)
...
(deftest executor-unresolved-stall-symbol-fails-loudly
  ...
  (with-runtime
    (fn [rt _]
      (test-support/activate-spool! rt :millhouse/workflow 'millhouse.workflow)
      (workflow/register-executor! :exec-gone 'millhouse.workflow-test/no-such-predicate)
      (let [err (is (thrown-with-msg? clojure.lang.ExceptionInfo
        ...
        (is (= 'millhouse.workflow-test/no-such-predicate (:stalled? (ex-data err)))))
  ...
  (with-runtime
    (fn [rt _]
      ...
      (workflow/register-executor!
       :decl-exec {:stalled? 'millhouse.workflow-test/exec-detail-a
      ...
      (let [item (first (filter #(= "decl-exec" (:waiter %))
        ...
        (is (= "millhouse.workflow-test/exec-detail-a" (:stall-predicate item)))
        (is (= "millhouse.workflow-test/decl-exec-request"
  ...
  (with-runtime
    (fn [rt _]
      ...
      (is (thrown-with-msg? clojure.lang.ExceptionInfo #"Executor declaration map"
                            (workflow/register-executor!
                             :bad-decl {:stalledd? 'millhouse.workflow-test/exec-detail-a}))))))
...
(deftest executor-catalog-fails-loudly-on-an-unresolvable-request-spec
  ...
  (with-runtime
    (fn [rt _]
      (test-support/activate-spool! rt :millhouse/workflow 'millhouse.workflow)
      (workflow/register-executor!
       :stale-decl {:stalled? 'millhouse.workflow-test/exec-detail-a
      ...
      (let [handle (wf-registry/registry-handle rt)
            ...
            spools-executors (fn [entries]
                               (registry/replace-owner!
                                ...
                                {:layer :spools :entries entries :overrides #{}}))]
        (spools-definitions {:route-a 'millhouse.workflow-test/registry-second-stage
                             :route-b 'millhouse.workflow-test/registry-alt-second-stage})
        (spools-executors {"exec-a" 'millhouse.workflow-test/exec-detail-a
                           "exec-b" 'millhouse.workflow-test/exec-detail-b})
    ...
    (fn [rt _]
      ...
      (let [handle (wf-registry/registry-handle rt)]
        (registry/replace-owner! handle workflow/definition-kind :spools/pkg
                                 {:layer :spools
                                  :entries {:route-x 'millhouse.workflow-test/registry-second-stage}
                                  :overrides #{}})
        (is (= 'millhouse.workflow-test/registry-second-stage
               (workflow/workflow-definition :route-x)))
        ;; a direct/REPL registration shadows the lower spools layer
        (workflow/register-workflow! :route-x 'millhouse.workflow-test/registry-alt-second-stage)
        (is (= 'millhouse.workflow-test/registry-alt-second-stage
               (workflow/workflow-definition :route-x))
...
(deftest static-definition-start-merges-defaults-and-records-identity
  (with-runtime
    (fn [rt _]
      (test-support/activate-spool! rt :millhouse/workflow 'millhouse.workflow)
      (workflow/register-workflow! :wt-build 'millhouse.workflow-test/static-build)
      ...
      (let [root (workflow/current-root "static-run")]
        ...
        (is (= "millhouse.workflow-test/static-build"
               (get-in root [:attributes :workflow/definition]))
...
(deftest static-definition-start-requires-the-start-entrypoint
  (with-runtime
    (fn [rt _]
      (test-support/activate-spool! rt :millhouse/workflow 'millhouse.workflow)
      (workflow/register-workflow! :wt-review 'millhouse.workflow-test/static-review)
...
(deftest unregister-workflow-removes-the-direct-registration
  (with-runtime
    (fn [rt _]
      ...
      (workflow/register-workflow! :wt-build 'millhouse.workflow-test/static-build)
      (workflow/register-workflow! :wt-review 'millhouse.workflow-test/static-review)
      (is (= {:wt-review 'millhouse.workflow-test/static-review}
  ...
  (with-runtime
    (fn [rt _]
      ...
      (let [thrown (try (workflow/register-workflow!
                         :wt-missing 'millhouse.workflow-test/no-such-definition)
                        (catch clojure.lang.ExceptionInfo e e))
            data (ex-data thrown)]
        (is (= :workflow/definition-unresolvable (:reason data)))
        (is (= :wt-missing (:name data)))
        (is (= 'millhouse.workflow-test/no-such-definition (:definition data)))
        (is (= 'millhouse.workflow-test (:namespace data)))
        (is (seq (:repair data))))
      (is (empty? (workflow/workflows))
...
(deftest register-workflow-rejects-a-route-to-a-missing-target
  (with-runtime
    (fn [rt _]
      (test-support/activate-spool! rt :millhouse/workflow 'millhouse.workflow)
      (let [thrown (try (workflow/register-workflow!
                         :wt-spike 'millhouse.workflow-test/static-spike)
                        (catch clojure.lang.ExceptionInfo e e))]
...
(deftest register-workflow-rejects-non-json-defaults-and-unknown-param-specs
  (with-runtime
    (fn [rt _]
      (test-support/activate-spool! rt :millhouse/workflow 'millhouse.workflow)
      (let [thrown (try (workflow/register-workflow!
                         :wt-bad-defaults 'millhouse.workflow-test/bad-defaults-definition)
                        (catch clojure.lang.ExceptionInfo e e))]
        (is (= :workflow/defaults-invalid (:reason (ex-data thrown))))
        (is (= [:at] (:path (ex-data thrown)))))
      (let [thrown (try (workflow/register-workflow!
                         :wt-bad-spec 'millhouse.workflow-test/bad-spec-definition)
                        (catch clojure.lang.ExceptionInfo e e))]
...
(deftest a-registered-symbol-must-resolve-to-a-definition-map
  ...
  (with-runtime
    (fn [rt _]
      (test-support/activate-spool! rt :millhouse/workflow 'millhouse.workflow)
      (doseq [[label sym] [["a function" 'millhouse.workflow-test/exploding-constructor]
                           ["a non-workflow value" 'millhouse.workflow-test/malformed-constructor]]]
...
(deftest revision-resolves-the-live-registered-name
  (with-runtime
    (fn [rt _]
      (test-support/activate-spool! rt :millhouse/workflow 'millhouse.workflow)
      (workflow/register-workflow! :wt-revisable 'millhouse.workflow-test/revisable-definition)
      ...
      (workflow/register-workflow! :wt-revisable 'millhouse.workflow-test/revisable-definition-v2)
      (workflow/choose! "revise-name-run" :again)
      (is (= "Revisable v2" (:title (workflow/current-root "revise-name-run"))))
      (is (= "millhouse.workflow-test/revisable-definition-v2"
  ...
  (with-runtime
    (fn [rt _]
      ...
      (let [original @#'live-var-definition]
        (try
          (workflow/register-workflow! :wt-live 'millhouse.workflow-test/live-var-definition)
...
(deftest spec-first-params-merge-defaults-before-whole-map-validation
  (with-runtime
    (fn [rt _]
      (test-support/activate-spool! rt :millhouse/workflow 'millhouse.workflow)
      (workflow/register-workflow! :wt-spec-build 'millhouse.workflow-test/spec-first-build)
...
(deftest spec-first-revision-validates-its-override-params
  (with-runtime
    (fn [rt _]
      (test-support/activate-spool! rt :millhouse/workflow 'millhouse.workflow)
      (workflow/register-workflow! :wt-revisable 'millhouse.workflow-test/spec-first-revisable)
    ...
    (fn [rt _]
      ...
      (testing "the poured checkpoint records identity, doc, and the form graph"
        (let [detail (workflow/choice-detail "input-run" :approve)
          ...
          (is (= "millhouse.workflow-test/approval-input" (get declared "spec")))
          ...
          (is (= [{"spec" "millhouse.workflow-test/approval-input"
                   "relation" "root"
                   "form" (pr-str (s/form ::approval-input))}
                  {"spec" "millhouse.workflow-test/approval-note"
...
(deftest definition-view-merges-authored-docs-and-example
  (with-runtime
    (fn [rt _]
      (test-support/activate-spool! rt :millhouse/workflow 'millhouse.workflow)
      (workflow/register-workflow! :wt-authored 'millhouse.workflow-test/authored-build)
      (let [params (:params (workflow/definition-view :wt-authored))
        ...
        (testing "a definition without authored fields shows none"
          (workflow/register-workflow!
           :wt-plain 'millhouse.workflow-test/spec-first-build)
...
(deftest registering-a-definition-with-an-unknown-input-spec-is-refused
  (with-runtime
    (fn [rt _]
      (test-support/activate-spool! rt :millhouse/workflow 'millhouse.workflow)
      (let [thrown (try (workflow/register-workflow!
                         :wt-bad-input 'millhouse.workflow-test/unknown-input-definition)
                        (catch clojure.lang.ExceptionInfo e e))]
        (is (= :workflow/input-spec-missing (:reason (ex-data thrown))))
...
(deftest spec-forms-walks-nested-keys-collections-and-aliases
  (is (= [["millhouse.workflow-test/node" "root"]
          ["millhouse.workflow-test/children" "keyword-reference"]
          ["millhouse.workflow-test/label" "keyword-reference"]]
         (form-graph ::node))
  ...
  (is (= [["millhouse.workflow-test/node-alias" "root"]
          ["millhouse.workflow-test/children" "keyword-reference"]
          ["millhouse.workflow-test/label" "keyword-reference"]
          ["millhouse.workflow-test/node" "keyword-reference"]]
...
(deftest spec-forms-is-cycle-safe-and-deterministic
  ...
  (is (= 1 (count (filter #(= "millhouse.workflow-test/node" (first %))
                          (form-graph ::node))))))

(deftest spec-forms-reports-keyword-literals-without-claiming-dependency
  (is (= [["millhouse.workflow-test/stage-enum" "root"]
          ["millhouse.workflow-test/draft" "keyword-reference"]]
...
(deftest spec-forms-executes-no-predicate
  ...
  (is (= [["millhouse.workflow-test/counting-params" "root"]
          ["millhouse.workflow-test/counted" "keyword-reference"]]
...
(deftest registered-call-targets-validate-their-params
  ...
  (with-runtime
    (fn [rt _]
      (test-support/activate-spool! rt :millhouse/workflow 'millhouse.workflow)
      (workflow/register-workflow! :wt-callable 'millhouse.workflow-test/spec-first-callable)
...
(defn- register-defer-targets! []
  (workflow/register-workflow! :wt-devflow 'millhouse.workflow-test/defer-devflow)
  (workflow/register-workflow! :wt-spike 'millhouse.workflow-test/defer-spike))
...
(deftest registering-a-definition-with-an-unbound-defer-is-refused
  (with-runtime
    (fn [rt _]
      (test-support/activate-spool! rt :millhouse/workflow 'millhouse.workflow)
      (let [thrown (try (workflow/register-workflow!
                         :wt-template 'millhouse.workflow-test/card-template)
                        (catch clojure.lang.ExceptionInfo e e))]
        (is (= :workflow/defer-unbound (:reason (ex-data thrown)))))
  ...
  (with-runtime
    (fn [rt _]
      ...
      (testing "a target that is not registered at all"
        (let [thrown (try (workflow/register-workflow!
                           :wt-card 'millhouse.workflow-test/tracked-card)
                          (catch clojure.lang.ExceptionInfo e e))]
          (is (= :workflow/reference-unregistered (:reason (ex-data thrown))))
          (is (= :call (:entrypoint (ex-data thrown)))
      ...
      (testing "a target that declares only :continue is not selectable"
        (workflow/register-workflow! :wt-continueonly
                                     'millhouse.workflow-test/defer-continue-only)
        (let [thrown (try (workflow/register-workflow!
                           :wt-bad-card
      ...
      (testing "a target that is not a definition map at all"
        (let [thrown (try (workflow/register-workflow!
                           :wt-legacy 'millhouse.workflow-test/exploding-constructor)
                          (catch clojure.lang.ExceptionInfo e e))]
          (is (= :workflow/definition-invalid (:reason (ex-data thrown))))
          (is (= 'millhouse.workflow-test/exploding-constructor
...
(deftest a-call-procedure-may-declare-a-defer
  ...
  (with-runtime
    (fn [rt _]
      (test-support/activate-spool! rt :millhouse/workflow 'millhouse.workflow)
      (register-defer-targets!)
      (workflow/register-workflow! :wt-card 'millhouse.workflow-test/tracked-card)
      (is (= :wt-caller (workflow/register-workflow!
                         :wt-caller 'millhouse.workflow-test/defer-caller)))
  ...
  (with-runtime
    (fn [rt _]
      ...
      (workflow/register-workflow! :wt-two-step
                                   'millhouse.workflow-test/defer-two-step-target)
          ...
          (let [filled (workflow/defer! "sandwich" :wt-two-step
            ...
            (testing "CC6: the fill record"
              (let [attrs (:attributes join)]
                ...
                (is (= "millhouse.workflow-test/defer-two-step-target"
  ...
  (with-runtime
    (fn [rt _]
      ...
      (testing "a compatible repoint runs the replacement"
        (workflow/register-workflow! :wt-spike 'millhouse.workflow-test/defer-devflow)
...
(deftest defer-refuses-a-direct-cycle-and-permits-siblings
  ...
  (with-runtime
    (fn [rt _]
      ...
      (workflow/register-workflow! :wt-two-step
                                   'millhouse.workflow-test/defer-two-step-target)
      (workflow/register-workflow! :wt-self 'millhouse.workflow-test/defer-self-target)
  ...
  (with-runtime
    (fn [rt _]
      ...
      (workflow/register-workflow! :wt-nested-inner
                                   'millhouse.workflow-test/defer-nested-inner)
      (workflow/register-workflow! :wt-nested-outer
                                   'millhouse.workflow-test/defer-nested-outer)
  ...
  (with-runtime
    (fn [rt _]
      ...
      (workflow/register-workflow! :wt-cycle-b
                                   'millhouse.workflow-test/defer-two-step-target)
      (workflow/register-workflow! :wt-cycle-a 'millhouse.workflow-test/defer-cycle-a)
      (workflow/register-workflow! :wt-cycle-b 'millhouse.workflow-test/defer-cycle-b)
...
(deftest defer-materializes-a-multi-step-expansion-with-colliding-ids
  ...
  (with-runtime
    (fn [rt _]
      (test-support/activate-spool! rt :millhouse/workflow 'millhouse.workflow)
      (workflow/register-workflow! :wt-fanout 'millhouse.workflow-test/defer-fanout-target)
...
(deftest defer-into-an-empty-target-does-not-stall-the-run
  ...
  (with-runtime
    (fn [rt _]
      (test-support/activate-spool! rt :millhouse/workflow 'millhouse.workflow)
      (workflow/register-workflow! :wt-empty 'millhouse.workflow-test/defer-empty-target)
    ...
    (fn [rt _]
      ...
      (let [pending (workflow/ready-step "atomic")
        ...
        (hooks/register-hook! rt :reject-defer #{:batch/apply-before-commit}
                              'millhouse.workflow-test/reject-defer-batch-hook {})
  ...
  (with-runtime
    (fn [rt _]
      ...
      (workflow/register-workflow! :wt-second
                                   'millhouse.workflow-test/registry-second-stage)
      (let [definition (workflow/bind-defers
                        (workflow/workflow
...
(deftest executor-authoring-validates-closed-options
  (is (= {:stalled? 'millhouse.workflow-test/exec-detail-a
          :request-spec ::request}
         (workflow/executor-declaration
          {:request-spec ::request}
          'millhouse.workflow-test/exec-detail-a)))

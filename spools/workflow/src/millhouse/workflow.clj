(ns millhouse.workflow
  ...
  (:require [clojure.spec.alpha :as s]
            ...
            [millhouse.workflow.internal.runs :as runs]
            [millhouse.workflow.internal.specs :as specs]
            [millhouse.workflow.internal.util :as util]))
  ...
  ([run-id opts]
   (let [rt (current/runtime)]
     ...
     (when (contains? opts :attributes)
       (require-valid! :millhouse.workflow.request/attributes (:attributes opts)
                       "Invalid workflow complete attributes"))
     (let [context (when (contains? opts :context)
                     (let [normalized
                           (-> (require-valid! :millhouse.workflow.values/params
                                               (:context opts)
                                               "Invalid workflow complete context")
                               cmp/default-context)]
                       (require-valid! :millhouse.workflow.request/context
                                       normalized
                                       "Invalid workflow complete context")))]
       (guard/with-run!
         rt run-id
...
(s/def ::executor-entry
  :millhouse.workflow.internal.registry/executor-entry)
...
(authoring/register-registry-kind!
 definition-kind :millhouse.workflow.internal.registry/definition-symbol)
...
(defn register-executor!
  ...
  (cond
    ...
    (map? pred) (do (when-not (s/valid? :millhouse.workflow.internal.registry/executor-decl pred)
                      (fail! "Executor declaration map must carry :stalled? (and optionally :request-spec)"
                             {:waiter waiter :pred pred
                              :explain (s/explain-str
                                        :millhouse.workflow.internal.registry/executor-decl pred)}))
...
(defn retry-validation!
  ...
  [request]
  ((requiring-resolve 'millhouse.executors.shell/retry-validation!) request))
...
(runtime/collect-kind!
 :millhouse.workflow.internal.registry/registry
 {:id definition-kind
  :entry-spec :millhouse.workflow.internal.registry/definition-symbol
...
(runtime/collect-kind!
 :millhouse.workflow.internal.registry/registry
 {:id executor-kind
  :entry-spec :millhouse.workflow.internal.registry/executor-entry
...
(defn- non-blank-string?
  ...
  ;; spec predicates below resolve their symbol to `millhouse.workflow`,
...
(s/def :millhouse.workflow.values/params (s/map-of keyword? any?))
(s/def ::count pos-int?)
(s/def ::chain boolean?)
...
(s/def ::call (s/keys :req-un [::id ::procedure]
                      :opt-un [::title :millhouse.workflow.values/params
...
(s/def ::revise (s/keys :req-un [:millhouse.workflow.values/params]))
(s/def ::choice
  (s/or :key ::id-ref
...
(s/def :millhouse.workflow.request/run-id non-blank-string?)
(s/def :millhouse.workflow.request/workflow ::registry-name)
(s/def :millhouse.workflow.request/params :millhouse.workflow.values/params)
(s/def :millhouse.workflow.request/step non-blank-string?)
(s/def :millhouse.workflow.request/by-identity non-blank-string?)
(s/def :millhouse.workflow.request/executor non-blank-string?)
(s/def :millhouse.workflow.request/executor-run-id non-blank-string?)
...
(s/def :millhouse.workflow.request/attributes (s/map-of non-blank-string? any?))
(s/def ::defer-request
  (s/keys :req-un [:millhouse.workflow.request/run-id
                   :millhouse.workflow.request/workflow]
          :opt-un [:millhouse.workflow.request/params
                   :millhouse.workflow.request/step
                   :millhouse.workflow.request/by-identity]))

;; --- discovery request and projection shapes ------------------------------

(s/def :millhouse.workflow.request/entrypoint ::entrypoint)
(s/def ::list-request
  (s/keys :opt-un [:millhouse.workflow.request/entrypoint]))
(s/def ::show-request (s/keys :req-un [:millhouse.workflow.request/workflow]))
...
(s/def :millhouse.workflow.view/name non-blank-string?)
(s/def :millhouse.workflow.view/doc non-blank-string?)
(s/def :millhouse.workflow.view/entrypoints
  (s/coll-of (into #{} (map name) defs/entrypoint-order) :kind vector?))
(s/def :millhouse.workflow.view/definition non-blank-string?)
(s/def :millhouse.workflow.view/step non-blank-string?)
...
(s/def ::catalog-fields
  (s/keys :req-un [:millhouse.workflow.view/name
                   :millhouse.workflow.view/doc
                   :millhouse.workflow.view/entrypoints
                   :millhouse.workflow.view/definition]))
(s/def ::catalog-item
...
(s/def :millhouse.workflow.view.params/kind #{"spec" "none"})
(s/def :millhouse.workflow.view.params/spec non-blank-string?)
(s/def :millhouse.workflow.view.params/spec-form
...
(s/def :millhouse.workflow.view.params/spec-forms
  (s/coll-of :millhouse.workflow.view.params/spec-form :kind vector?))
(s/def :millhouse.workflow.view.params/defaults (s/map-of keyword? any?))
(s/def :millhouse.workflow.view.params/contract (s/map-of string? any?))
(s/def :millhouse.workflow.view.params/template any?)
(s/def :millhouse.workflow.view.params/example ::example)
(s/def :millhouse.workflow.view/params
  (s/keys :req-un [:millhouse.workflow.view.params/kind]
          :opt-un [:millhouse.workflow.view.params/spec
                   :millhouse.workflow.view.params/spec-forms
                   :millhouse.workflow.view.params/contract
                   :millhouse.workflow.view.params/template
                   :millhouse.workflow.view.params/defaults
                   :millhouse.workflow.view.params/example]))
...
(s/def :millhouse.workflow.view.declared/kind #{"static"})
(s/def :millhouse.workflow.view.declared/each non-blank-string?)
(s/def :millhouse.workflow.view.declared/waiter non-blank-string?)
(s/def :millhouse.workflow.view.declared/procedure non-blank-string?)
...
(s/def :millhouse.workflow.view.call/kind
  #{"registered" "symbol" "var" "inline"})
(s/def :millhouse.workflow.view.declared/defer non-blank-string?)
(s/def :millhouse.workflow.view.declared/entrypoint #{"call"})
(s/def :millhouse.workflow.view.declared/names
  (s/coll-of non-blank-string? :kind vector?))
(s/def :millhouse.workflow.view.declared/entry
  :millhouse.workflow.view.declared/names)
(s/def :millhouse.workflow.view.declared/choices
  :millhouse.workflow.view.declared/names)
(s/def :millhouse.workflow.view.declared/workflows
  :millhouse.workflow.view.declared/names)
(s/def :millhouse.workflow.view.declared/routes
  :millhouse.workflow.view.declared/names)
(s/def :millhouse.workflow.view.declared/loop
  (s/keys :req-un [:millhouse.workflow.view/step]
          :opt-un [:millhouse.workflow.view.declared/each ::count ::chain]))
(s/def :millhouse.workflow.view.declared/loops
  (s/coll-of :millhouse.workflow.view.declared/loop :kind vector?))
(s/def :millhouse.workflow.view.declared/gate
  (s/keys :req-un [:millhouse.workflow.view/step
                   :millhouse.workflow.view.declared/waiter]))
(s/def :millhouse.workflow.view.declared/gates
  (s/coll-of :millhouse.workflow.view.declared/gate :kind vector?))
(s/def :millhouse.workflow.view.declared/checkpoint
  (s/keys :req-un [:millhouse.workflow.view/step
                   :millhouse.workflow.view.declared/choices]))
(s/def :millhouse.workflow.view.declared/checkpoints
  (s/coll-of :millhouse.workflow.view.declared/checkpoint :kind vector?))
(s/def :millhouse.workflow.view.declared/call
  (s/keys :req-un [:millhouse.workflow.view/step
                   :millhouse.workflow.view.declared/procedure
                   :millhouse.workflow.view.call/kind]))
(s/def :millhouse.workflow.view.declared/calls
  (s/coll-of :millhouse.workflow.view.declared/call :kind vector?))
(s/def :millhouse.workflow.view.declared/defer-point
  (s/keys :req-un [:millhouse.workflow.view/step
                   :millhouse.workflow.view.declared/defer
                   :millhouse.workflow.view.declared/workflows
                   :millhouse.workflow.view.declared/entrypoint]))
(s/def :millhouse.workflow.view.declared/defers
  (s/coll-of :millhouse.workflow.view.declared/defer-point :kind vector?))
(s/def :millhouse.workflow.view/declared
  (s/keys :req-un [:millhouse.workflow.view.declared/kind]
          :opt-un [:millhouse.workflow.view.declared/entry
                   :millhouse.workflow.view.declared/loops
                   :millhouse.workflow.view.declared/gates
                   :millhouse.workflow.view.declared/checkpoints
                   :millhouse.workflow.view.declared/calls
                   :millhouse.workflow.view.declared/defers
                   :millhouse.workflow.view.declared/routes]))
...
(s/def ::definition-view
  (s/and (s/merge ::catalog-fields
                  (s/keys :req-un [:millhouse.workflow.view/params
                                   :millhouse.workflow.view/declared]))
...
(s/def :millhouse.workflow.view.executor/waiter non-blank-string?)
(s/def :millhouse.workflow.view.executor/stall-predicate
  (s/nilable non-blank-string?))
(s/def :millhouse.workflow.view.executor/request
  (s/keys :req-un [:millhouse.workflow.view.params/spec
                   :millhouse.workflow.view.params/spec-forms
                   :millhouse.workflow.view.params/contract
                   :millhouse.workflow.view.params/template]))
(s/def ::executor-view
...
(s/def :millhouse.workflow.request/choice non-blank-string?)
(s/def :millhouse.workflow.request/input :millhouse.workflow.values/params)
(s/def :millhouse.workflow.request/context
  (s/and :millhouse.workflow.values/params json-safe-context?))
(s/def :millhouse.workflow.request/timeout-secs ::timeout-secs)

(s/def :millhouse.workflow.advance/choice
  #(or (keyword? %) (non-blank-string? %)))
(s/def ::advance-opts
  (s/keys :opt-un [:millhouse.workflow.advance/choice
                   :millhouse.workflow.request/input
                   :millhouse.workflow.request/step
                   :millhouse.workflow.request/by-identity
                   :millhouse.workflow.request/attributes]))
...
(s/def ::choices-request
  (s/keys :req-un [:millhouse.workflow.request/run-id]
          :opt-un [:millhouse.workflow.request/step]))
(s/def ::complete-request
  (s/and
   (s/keys :req-un [:millhouse.workflow.request/run-id]
           :opt-un [:millhouse.workflow.request/step
                    :millhouse.workflow.request/by-identity
                    :millhouse.workflow.request/executor
                    :millhouse.workflow.request/executor-run-id
                    :millhouse.workflow.request/attributes
                    :millhouse.workflow.request/context])
   #(or (not (contains? % :executor-run-id))
        (contains? % :executor))))
(s/def ::choose-request
  (s/keys :req-un [:millhouse.workflow.request/run-id
                   :millhouse.workflow.request/choice]
          :opt-un [:millhouse.workflow.request/input
                   :millhouse.workflow.request/step
                   :millhouse.workflow.request/by-identity]))
(s/def ::next-request
  (s/keys :req-un [:millhouse.workflow.request/run-id]
          :opt-un [:millhouse.workflow.request/choice
                   :millhouse.workflow.request/input
                   :millhouse.workflow.request/step
                   :millhouse.workflow.request/by-identity]))
(s/def ::await-request
  (s/keys :req-un [:millhouse.workflow.request/run-id]
          :opt-un [:millhouse.workflow.request/timeout-secs]))
...
(s/def :millhouse.workflow.view/id non-blank-string?)
(s/def :millhouse.workflow.view/title string?)
(s/def :millhouse.workflow.view/state ::state)
...
(s/def :millhouse.workflow.view/role #{"step" "checkpoint" "defer"})
(s/def :millhouse.workflow.view/run-id non-blank-string?)
(s/def :millhouse.workflow.view/operation non-blank-string?)
(s/def :millhouse.workflow.view/done boolean?)
(s/def :millhouse.workflow.view/gate non-blank-string?)
(s/def :millhouse.workflow.view/defer non-blank-string?)
(s/def :millhouse.workflow.view/workflows (s/coll-of non-blank-string?))
(s/def :millhouse.workflow.view/choices (s/coll-of non-blank-string?))

(s/def ::root-view
  (s/keys :req-un [:millhouse.workflow.view/id
                   :millhouse.workflow.view/title
                   :millhouse.workflow.view/state]))
...
(defmethod ready-item-branch :checkpoint [_]
  (s/merge ::ready-fields (s/keys :req-un [:millhouse.workflow.view/choices])))

(defmethod ready-item-branch :defer [_]
  (s/merge ::ready-fields (s/keys :req-un [:millhouse.workflow.view/defer
                                           :millhouse.workflow.view/workflows])))

(defmethod ready-item-branch :gate [_]
  (s/merge ::ready-fields (s/keys :req-un [:millhouse.workflow.view/gate])))
...
(s/def :millhouse.workflow.view/root ::root-view)
(s/def :millhouse.workflow.view/ready (s/coll-of ::ready-item :kind vector?))
(s/def ::run-result
  (s/keys :req-un [:millhouse.workflow.view/operation
                   :millhouse.workflow.view/run-id
                   :millhouse.workflow.view/root
                   :millhouse.workflow.view/ready
                   :millhouse.workflow.view/done]))
...
(s/def :millhouse.workflow.view/reason
  #{:done :checkpoint :defer :step :gate :stalled :waiting :timeout})
(s/def :millhouse.workflow.view/detail map?)
(s/def ::attention-result
  (s/keys :req-un [:millhouse.workflow.view/operation
                   :millhouse.workflow.view/run-id
                   :millhouse.workflow.view/reason
                   :millhouse.workflow.view/ready
                   :millhouse.workflow.view/done]
          :opt-un [:millhouse.workflow.view/detail]))
...
(s/def :millhouse.workflow.view.choices/input-spec
...
(s/def :millhouse.workflow.view.choices/detail
  (s/and (s/map-of #{"label" "description" "next" "revise" "input-spec"} any?)
         ...
         #(every? (partial s/valid? :millhouse.workflow.view.choices/input-spec)
                  (vals (select-keys % ["input-spec"])))))
(s/def :millhouse.workflow.view.choices/choices
  (s/map-of non-blank-string? :millhouse.workflow.view.choices/detail))
(s/def ::choices-result
  (s/keys :req-un [:millhouse.workflow.view/operation
                   :millhouse.workflow.view/run-id
                   :millhouse.workflow.view.choices/choices]))
...
(defn- explain-workflow []
  {:topic :workflow
   ...
   :builders {'workflow 'millhouse.workflow/workflow
              'defworkflow 'millhouse.workflow/defworkflow
              'step 'millhouse.workflow/step
              'gate 'millhouse.workflow/gate
              'checkpoint 'millhouse.workflow/checkpoint
              'call 'millhouse.workflow/call
              'defer 'millhouse.workflow/defer
              'bind-defers 'millhouse.workflow/bind-defers}

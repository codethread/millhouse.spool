(ns millhouse.workflow.cli
  ...
  workspace that activates `millhouse.workflow` gets the engine, its
   ...
   {"list" {:type :map
    ...
    "executors" {:type :map
                 :required {:operation :string
                            ...
                            ;; request contract rides the millstrand.api.spec.alpha
                            ;; node grammar.
                            :executors :json}}
...
(def ^:private workflow-meta
  ...
  meta-verbs."
  {:about (fmt/reflow
           "|workflow is the worker surface over Millstrand's workflow engine. list
            ...
            |catalogue baked in when Millstrand was built. start, ready, complete,
...
(lifecycle/defseed workflow-glossary-seed
  ...
  {:apply 'millhouse.workflow.cli/seed-workflow-glossary!})

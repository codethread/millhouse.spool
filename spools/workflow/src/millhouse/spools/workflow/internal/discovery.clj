(ns millhouse.spools.workflow.internal.discovery
  ...
  *declares* — its entry items, loops, gates, checkpoints, calls, defers, and the
  ...
  (:require [millhouse.spools.workflow.internal.definitions :as defs]
            [millhouse.spools.workflow.internal.registry :as registry]
            [millhouse.spools.workflow.internal.specs :as specs]
            [millhouse.spools.workflow.internal.util :as util :refer [require-shape!]]))
...
(defn- catalog-item
  ...
  [resolved]
  (require-shape! :millhouse.spools.workflow/catalog-item
...
(defn catalog
  ...
  [rt request]
  (require-shape! :millhouse.spools.workflow/list-request request
...
(defn definition-view
  ...
  repair it."
  [rt name]
  (require-shape! :millhouse.spools.workflow/show-request {:workflow name}
  ...
  (let [resolved (defs/resolve-registered rt name)
    ...
    (require-shape! :millhouse.spools.workflow/definition-view
...
(defn executor-catalog
  ...
  (mapv (fn [[waiter {:keys [stalled? request-spec]}]]
          (require-shape! :millhouse.spools.workflow/executor-view
                          (cond-> {:waiter waiter

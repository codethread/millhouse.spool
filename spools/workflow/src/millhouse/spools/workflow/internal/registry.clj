(ns millhouse.spools.workflow.internal.registry
  ...
  owner-complete through `millstrand.api.registry.alpha`: a complete owner partition
...
(def executor-kind
  ...
  :millhouse.spools.workflow/executor)

(def ^:private repl-owner
  "Owner keyword for direct/REPL registrations, published in the `:direct` layer
  (DELTA-OlrDrt-001.CC3), matching the core registries' `:millstrand.owner/repl`."
  :millstrand.owner/repl)

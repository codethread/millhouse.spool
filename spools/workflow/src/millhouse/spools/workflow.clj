(ns millhouse.spools.workflow
  ...
  (:require [clojure.spec.alpha :as s]
            ...
            [millhouse.spools.workflow.internal.discovery :as discovery]
            [millhouse.spools.workflow.internal.guard :as guard]
            [millhouse.spools.workflow.internal.query :as query]
            [millhouse.spools.workflow.internal.registry :as registry]
            [millhouse.spools.workflow.internal.routing :as routing]
  ...
  ([run-id opts]
   (let [rt (current/runtime)]
     ...
     (let [context (when (contains? opts :context)
                     (let [normalized
                           (-> (require-valid! :millhouse.spools.workflow.values/params
                                               (:context opts)
                                               "Invalid workflow complete context")
                               cmp/default-context)]
                       (require-valid! :millhouse.spools.workflow.request/context
                                       normalized
                                       "Invalid workflow complete context")))]
       (guard/with-run!
         rt run-id
...
(s/def ::executor-entry
  :millhouse.spools.workflow.internal.registry/executor-entry)
...
(defn register-executor!
  ...
  (cond
    ...
    (map? pred) (do (when-not (s/valid? :millhouse.spools.workflow.internal.registry/executor-decl pred)
                      (fail! "Executor declaration map must carry :stalled? (and optionally :request-spec)"
                             {:waiter waiter :pred pred
                              :explain (s/explain-str
                                        :millhouse.spools.workflow.internal.registry/executor-decl pred)}))
...
(runtime/collect-kind!
 ...
 {:id definition-kind
  :entry-spec :millhouse.spools.workflow.internal.registry/definition-symbol
  :binding-moment :route-transition
  :candidate-validator
  'millhouse.spools.workflow.internal.definitions/validate-candidates!})
...
(defn- non-blank-string?
  ...
  ;; spec predicates below resolve their symbol to `millhouse.spools.workflow`,
...
(s/def :millhouse.spools.workflow.values/params (s/map-of keyword? any?))
(s/def ::count pos-int?)
(s/def ::chain boolean?)
...
(s/def ::call (s/keys :req-un [::id ::procedure]
                      :opt-un [::title :millhouse.spools.workflow.values/params
...
(s/def ::revise (s/keys :req-un [:millhouse.spools.workflow.values/params]))
(s/def ::choice
  (s/or :key ::id-ref
...
(s/def :millhouse.spools.workflow.request/run-id non-blank-string?)
...
(s/def :millhouse.spools.workflow.request/attributes (s/map-of non-blank-string? any?))
(s/def ::defer-request
...
(s/def :millhouse.spools.workflow.request/entrypoint ::entrypoint)
(s/def ::list-request
  (s/keys :opt-un [:millhouse.spools.workflow.request/entrypoint]))
(s/def ::show-request (s/keys :req-un [:millhouse.spools.workflow.request/workflow]))
...
(s/def :millhouse.spools.workflow.view/name non-blank-string?)
(s/def :millhouse.spools.workflow.view/doc non-blank-string?)
(s/def :millhouse.spools.workflow.view/entrypoints
  (s/coll-of (into #{} (map name) defs/entrypoint-order) :kind vector?))
(s/def :millhouse.spools.workflow.view/definition non-blank-string?)
(s/def :millhouse.spools.workflow.view/step non-blank-string?)
...
(s/def ::catalog-fields
  (s/keys :req-un [:millhouse.spools.workflow.view/name
                   :millhouse.spools.workflow.view/doc
                   :millhouse.spools.workflow.view/entrypoints
                   :millhouse.spools.workflow.view/definition]))
(s/def ::catalog-item
...
;; `:template` are the `millstrand.api.spec.alpha` nested and skeleton views
...
(s/def :millhouse.spools.workflow.view.params/kind #{"spec" "none"})
(s/def :millhouse.spools.workflow.view.params/spec non-blank-string?)
(s/def :millhouse.spools.workflow.view.params/spec-form
...
(s/def :millhouse.spools.workflow.view.params/spec-forms
  (s/coll-of :millhouse.spools.workflow.view.params/spec-form :kind vector?))
(s/def :millhouse.spools.workflow.view.params/defaults (s/map-of keyword? any?))
(s/def :millhouse.spools.workflow.view.params/contract (s/map-of string? any?))
(s/def :millhouse.spools.workflow.view.params/template any?)
(s/def :millhouse.spools.workflow.view.params/example ::example)
(s/def :millhouse.spools.workflow.view/params
  (s/keys :req-un [:millhouse.spools.workflow.view.params/kind]
          :opt-un [:millhouse.spools.workflow.view.params/spec
                   :millhouse.spools.workflow.view.params/spec-forms
                   :millhouse.spools.workflow.view.params/contract
                   :millhouse.spools.workflow.view.params/template
                   :millhouse.spools.workflow.view.params/defaults
                   :millhouse.spools.workflow.view.params/example]))
...
(s/def :millhouse.spools.workflow.view.declared/kind #{"static"})
(s/def :millhouse.spools.workflow.view.declared/each non-blank-string?)
(s/def :millhouse.spools.workflow.view.declared/waiter non-blank-string?)
(s/def :millhouse.spools.workflow.view.declared/procedure non-blank-string?)
...
(s/def :millhouse.spools.workflow.view.call/kind
  #{"registered" "symbol" "var" "inline"})
(s/def :millhouse.spools.workflow.view.declared/defer non-blank-string?)
(s/def :millhouse.spools.workflow.view.declared/entrypoint #{"call"})
(s/def :millhouse.spools.workflow.view.declared/names
  (s/coll-of non-blank-string? :kind vector?))
(s/def :millhouse.spools.workflow.view.declared/entry
  :millhouse.spools.workflow.view.declared/names)
(s/def :millhouse.spools.workflow.view.declared/choices
  :millhouse.spools.workflow.view.declared/names)
(s/def :millhouse.spools.workflow.view.declared/workflows
  :millhouse.spools.workflow.view.declared/names)
(s/def :millhouse.spools.workflow.view.declared/routes
  :millhouse.spools.workflow.view.declared/names)
(s/def :millhouse.spools.workflow.view.declared/loop
  (s/keys :req-un [:millhouse.spools.workflow.view/step]
          :opt-un [:millhouse.spools.workflow.view.declared/each ::count ::chain]))
(s/def :millhouse.spools.workflow.view.declared/loops
  (s/coll-of :millhouse.spools.workflow.view.declared/loop :kind vector?))
(s/def :millhouse.spools.workflow.view.declared/gate
  (s/keys :req-un [:millhouse.spools.workflow.view/step
                   :millhouse.spools.workflow.view.declared/waiter]))
(s/def :millhouse.spools.workflow.view.declared/gates
  (s/coll-of :millhouse.spools.workflow.view.declared/gate :kind vector?))
(s/def :millhouse.spools.workflow.view.declared/checkpoint
  (s/keys :req-un [:millhouse.spools.workflow.view/step
                   :millhouse.spools.workflow.view.declared/choices]))
(s/def :millhouse.spools.workflow.view.declared/checkpoints
  (s/coll-of :millhouse.spools.workflow.view.declared/checkpoint :kind vector?))
(s/def :millhouse.spools.workflow.view.declared/call
  (s/keys :req-un [:millhouse.spools.workflow.view/step
                   :millhouse.spools.workflow.view.declared/procedure
                   :millhouse.spools.workflow.view.call/kind]))
(s/def :millhouse.spools.workflow.view.declared/calls
  (s/coll-of :millhouse.spools.workflow.view.declared/call :kind vector?))
(s/def :millhouse.spools.workflow.view.declared/defer-point
  (s/keys :req-un [:millhouse.spools.workflow.view/step
                   :millhouse.spools.workflow.view.declared/defer
                   :millhouse.spools.workflow.view.declared/workflows
                   :millhouse.spools.workflow.view.declared/entrypoint]))
(s/def :millhouse.spools.workflow.view.declared/defers
  (s/coll-of :millhouse.spools.workflow.view.declared/defer-point :kind vector?))
(s/def :millhouse.spools.workflow.view/declared
  (s/keys :req-un [:millhouse.spools.workflow.view.declared/kind]
          :opt-un [:millhouse.spools.workflow.view.declared/entry
                   :millhouse.spools.workflow.view.declared/loops
                   :millhouse.spools.workflow.view.declared/gates
                   :millhouse.spools.workflow.view.declared/checkpoints
                   :millhouse.spools.workflow.view.declared/calls
                   :millhouse.spools.workflow.view.declared/defers
                   :millhouse.spools.workflow.view.declared/routes]))
...
(s/def ::definition-view
  (s/and (s/merge ::catalog-fields
                  (s/keys :req-un [:millhouse.spools.workflow.view/params
                                   :millhouse.spools.workflow.view/declared]))
...
(s/def :millhouse.spools.workflow.view.executor/waiter non-blank-string?)
(s/def :millhouse.spools.workflow.view.executor/stall-predicate
  (s/nilable non-blank-string?))
(s/def :millhouse.spools.workflow.view.executor/request
  (s/keys :req-un [:millhouse.spools.workflow.view.params/spec
                   :millhouse.spools.workflow.view.params/spec-forms
                   :millhouse.spools.workflow.view.params/contract
                   :millhouse.spools.workflow.view.params/template]))
(s/def ::executor-view
  (s/and (s/keys :req-un [:millhouse.spools.workflow.view.executor/waiter
                          :millhouse.spools.workflow.view.executor/stall-predicate]
                 :opt-un [:millhouse.spools.workflow.view.executor/request])
         #(every? #{:waiter :stall-predicate :request} (keys %))))

;; --- run lifecycle request and result shapes ------------------------------

(s/def :millhouse.spools.workflow.request/choice non-blank-string?)
(s/def :millhouse.spools.workflow.request/input :millhouse.spools.workflow.values/params)
(s/def :millhouse.spools.workflow.request/context
  (s/and :millhouse.spools.workflow.values/params json-safe-context?))
(s/def :millhouse.spools.workflow.request/timeout-secs ::timeout-secs)

(s/def :millhouse.spools.workflow.advance/choice
...
(s/def :millhouse.spools.workflow.view/id non-blank-string?)
(s/def :millhouse.spools.workflow.view/title string?)
(s/def :millhouse.spools.workflow.view/state ::state)
...
(s/def :millhouse.spools.workflow.view/role #{"step" "checkpoint" "defer"})
(s/def :millhouse.spools.workflow.view/run-id non-blank-string?)
(s/def :millhouse.spools.workflow.view/operation non-blank-string?)
(s/def :millhouse.spools.workflow.view/done boolean?)
(s/def :millhouse.spools.workflow.view/gate non-blank-string?)
(s/def :millhouse.spools.workflow.view/defer non-blank-string?)
(s/def :millhouse.spools.workflow.view/workflows (s/coll-of non-blank-string?))
(s/def :millhouse.spools.workflow.view/choices (s/coll-of non-blank-string?))

(s/def ::root-view
  (s/keys :req-un [:millhouse.spools.workflow.view/id
                   :millhouse.spools.workflow.view/title
                   :millhouse.spools.workflow.view/state]))
...
(s/def ::ready-fields
  (s/keys :req-un [:millhouse.spools.workflow.view/id
                   :millhouse.spools.workflow.view/role
                   :millhouse.spools.workflow.view/title]))
...
(defmethod ready-item-branch :checkpoint [_]
  (s/merge ::ready-fields (s/keys :req-un [:millhouse.spools.workflow.view/choices])))

(defmethod ready-item-branch :defer [_]
  (s/merge ::ready-fields (s/keys :req-un [:millhouse.spools.workflow.view/defer
                                           :millhouse.spools.workflow.view/workflows])))

(defmethod ready-item-branch :gate [_]
  (s/merge ::ready-fields (s/keys :req-un [:millhouse.spools.workflow.view/gate])))
...
(s/def :millhouse.spools.workflow.view/root ::root-view)
(s/def :millhouse.spools.workflow.view/ready (s/coll-of ::ready-item :kind vector?))
(s/def ::run-result
  (s/keys :req-un [:millhouse.spools.workflow.view/operation
                   :millhouse.spools.workflow.view/run-id
                   :millhouse.spools.workflow.view/root
                   :millhouse.spools.workflow.view/ready
                   :millhouse.spools.workflow.view/done]))
...
(s/def :millhouse.spools.workflow.view/reason
  #{:done :checkpoint :defer :step :gate :stalled :waiting :timeout})
(s/def :millhouse.spools.workflow.view/detail map?)
(s/def ::attention-result
  (s/keys :req-un [:millhouse.spools.workflow.view/operation
                   :millhouse.spools.workflow.view/run-id
                   :millhouse.spools.workflow.view/reason
                   :millhouse.spools.workflow.view/ready
                   :millhouse.spools.workflow.view/done]
          :opt-un [:millhouse.spools.workflow.view/detail]))
...
;; owned by `millstrand.api.spec.alpha`, so only its JSON carrier types are
;; constrained (spec-projection DELTA-Spj-003.CC2).
(s/def :millhouse.spools.workflow.view.choices/input-spec
...
(s/def :millhouse.spools.workflow.view.choices/detail
  (s/and (s/map-of #{"label" "description" "next" "revise" "input-spec"} any?)
         ...
         #(every? (partial s/valid? :millhouse.spools.workflow.view.choices/input-spec)
                  (vals (select-keys % ["input-spec"])))))
(s/def :millhouse.spools.workflow.view.choices/choices
  (s/map-of non-blank-string? :millhouse.spools.workflow.view.choices/detail))
(s/def ::choices-result
  (s/keys :req-un [:millhouse.spools.workflow.view/operation
                   :millhouse.spools.workflow.view/run-id
                   :millhouse.spools.workflow.view.choices/choices]))
...
(defn- explain-workflow []
  {:topic :workflow
   :summary "Clojure-native workflow data compiled into a Millstrand molecule or wisp."
   :builders {'workflow 'millhouse.spools.workflow/workflow
              'defworkflow 'millhouse.spools.workflow/defworkflow
              'step 'millhouse.spools.workflow/step
              'gate 'millhouse.spools.workflow/gate
              'checkpoint 'millhouse.spools.workflow/checkpoint
              'call 'millhouse.spools.workflow/call
              'defer 'millhouse.spools.workflow/defer
              'bind-defers 'millhouse.spools.workflow/bind-defers}

...
(def ^:private timeout-handler
  'millhouse.executors.shell/timeout-wake)
...
(lifecycle/defreconcile shell-attempts
  ...
  {:read-desired 'millhouse.executors.shell/read-shell-attempts
   :read-actual 'millhouse.executors.shell/read-shell-custody
   :apply 'millhouse.executors.shell/apply-shell-attempts!
   :on-removed 'millhouse.executors.shell/remove-shell-attempts!
   :after #{:shell-handler}})
...
(defn- register-shell-handler!
  ...
  (events/register-handler! runtime :shell/engine event-types
                            'millhouse.executors.shell/on-event
                            {:spool "shell"}))
...
(lifecycle/defresource shell-pool
  ...
  {:open 'millhouse.executors.shell/open-shell-pool!
   :close 'millhouse.executors.shell/close-shell-pool!
   :scope :runtime})

(lifecycle/defresource shell-handler
  "Own the shell event handler for the lifetime of the module."
  {:open 'millhouse.executors.shell/open-shell-handler!
   :close 'millhouse.executors.shell/close-shell-handler!
   :after #{:shell-pool}})

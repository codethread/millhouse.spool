...
`millhouse.spools.workflow` is a Clojure-native workflow layer built on ordinary Millstrand strands, edges, and batch mutations. It lets spool authors define small workflow molecules that agents can execute one step at a time without needing to understand the underlying graph engine.
...
The returning-defer change is a cold cutover with no old-strand interpreter. Before installing a build that contains it, follow the [defer-return cutover runbook](https://github.com/codethread/millstrand/blob/aed95c22bbdb1fe5a916886e8ebda787d370173d/docs/spools/defer-return-cutover.md); live refresh is not a valid pickup path.
...
Use `defworkflow` in module source for a workflow that should survive refresh and restart. The direct `register-workflow!` and `register-executor!` functions are the runtime-implied seam for trusted code and tests, and the `workflow` CLI reads the effective registry. Direct registration is runtime-lifetime state; `unregister-workflow!` removes only the direct entry and never touches the Var or a module-owned declaration. If a module should stop publishing a workflow, omit its `defworkflow` form and refresh.
...
- The separate `millhouse.spools.executors/shell` root fulfills ready `:shell`
...
(workflow/register-executor! :shell {:stalled? 'millhouse.spools.executors.shell/gate-stalled?
                                     :request-spec :millhouse.spools.executors.shell/request})
...
The CLI over these reads is a module of its own. Activating `millhouse.spools.workflow` gives you the engine, its registries, and its Clojure API — and no CLI verbs. The `workflow` op appears only when startup config also declares the CLI module:
...
{"operation":"workflow executors",
 "executors":[{"waiter":"shell",
               "stall-predicate":"millhouse.spools.executors.shell/gate-stalled?",
               "request":{"spec":"millhouse.spools.executors.shell/request",
...
This table is the extension API: spools built on top of `millhouse.spools.workflow` (like `ct.spools.devflow`) read and write these `workflow/*` attributes directly on strands. Unless noted, attributes are plain string-keyed `TEXT`/JSON values on the strand's `:attributes` map.
...
- `(millhouse.spools.workflow/explain)` / `(explain topic)` — machine-readable
...
- [`millhouse.spools.executors.shell`](../shell-executor/README.md) — separate-root executor that fulfills
